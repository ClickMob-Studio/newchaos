...
if ($user_class->gang != 0){
        ...
        $db->execute(array(
                ...
                $db->query("UPDATE grpgusers SET `both` = `both` + 1, `epoints` = `epoints` + `eventbusts`, `bustcomp` = `bustcomp` + 1, exp = exp + ?, busts = busts + 1, nerve = nerve - ? WHERE id = ?");
                $db->execute(array(
                    $exp,
                    $nerve,
                    $user_class->id
