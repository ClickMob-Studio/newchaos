...
$menus = array(
    ...
    1 => array(
        'title' => 'Raids <span class="notify">[beta]</span>',
        'url' => 'raids.php'
    ),
    2 => array(
        'title' => 'Daily Jobs',
...
echo '</div>';
echo '</div>';
if($user_class->admin >0 || $user_class->id == 9){
    echo '<li class="ui-state-default"><a href="staff_sell_log.php">Sell Log</a></li>';
}
echo '</div>';
echo '</div>';
...
if ($user_class->cityturns >= 50) {
    echo '<a href="maze.php"><span style="color:red;">You have MAX Turns to search the maze!.</span></a><br />';
}
...
if (time() <  1673827199) {
    ...
    echo '<br><div class="pulsate" style="font-family:timesnewroman;font-size: 2em;color:1e7b00;text-align: center;margin-bottom: 20px;margin-top: -20px;"><a href=newcrimes.php><font colour=purple>Double EXP ACTIVE on all Crimes!</a>
...
if (time() <  1661122799) {
    ...
    echo '<div class="pulsate" style="font-family:TimesNewroman;font-size: 1.5em;color:1e7b00;text-align: center;margin-bottom: 20px;margin-top: -20px;"><a href=bustcontest.php>Our BUST COMPETITION IS currently active! </a>
...
if ($user_class->claimed == 0 && basename($_SERVER['PHP_SELF']) != 'rmstore.php') {    // The original echo statement for the claim message should be commented out or removed
    ...
    <h2>Happy Festive Season</h2>
    <p>Hope you all enjoy a great christmas period and enjoy the many upcoming changes</p>

    <p>You've received a free Christmas Gift package:</p>
    <ul class="gift-list">
...
if ($bonus_row['Time'] > 0) {
    ...
    echo '<div style="font-family:Arial;font-size: 2.0em;color:red;text-align: center;margin-bottom: 20px;margin-top: -20px;"><a href="bonuspot.php"><font color=green>Server Wide Double EXP Active: </font>  <font color=white>' . $_tt . '</font> </a>
                                                </div>';
}
...
if ($userInfo['dailyClockins'] < 8 && $jobInfo['lastClockin'] < time() - 3600) {
  ...
  echo '<div style="font-family:Arial;font-size: 2.5em;color:red;text-align: center;margin-bottom: 20px;margin-top: -20px;"><a href="jobs.php"><font color=white>You have not Clocked in this hour! </font> </a>
                                                </div>';
}
...
if (!mysql_num_rows($result)) {
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        ?>
                        <li class="slide">
                            <div class="slide-content">
                                <span class="<?php echo ($row['glow'] == 1 ? "glow-text" : "") ?>"><?php echo $user_ads->formattedname ?>: <?php echo $row['message'] ?></span>
                            </div>
