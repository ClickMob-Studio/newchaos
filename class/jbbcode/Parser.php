...
class Parser {
	...
	public function loadDefaultCodes() {
		...
		$this->addBBCode("center", "<span style='text-align:center;'>{param}</span>");
		$this->addBBCode("i", "<em>{param}</em>");
		$this->addBBCode("u", "<u>{param}</u>");
		$this->addBBCode("url", "<a href='{param}' class='bbcodeURL'>{param}</a> (click here)");
		$this->addBBCode("url", "<a href='{option}' class='bbcodeURL'>{param}</a> (click here)", true);
		$this->addBBCode("urlnew", "<a href='{param}' target='_new' class='bbcodeURLnew'>{param}</a> (opens in new tab/window)");
		$this->addBBCode("urlnew", "<a href='{option}' target='_new' class='bbcodeURLnew'>{param}</a> (opens in new tab/window)", true);
		$this->addBBCode("color", str_replace('[hash]', '#', "<span style='color: {option};'>{param}</span>"), true);
		$this->addBBCode("br", '<br />{param}');
		$this->addBBCode("quote", '<div class="quotetop">QUOTE</div><div class="quotemain">{param}</div>');
		$this->addBBCode("quote", '<div class="quotetop">QUOTE ({option})</div><div class="quotemain">{param}</div>', true);
		$this->addBBCode("code", '<div class="codetop">CODE</div><div class="codemain">{param}</div>');
		$this->addBBCode("code", '<div class="codetop">CODE ({option})</div><div class="codemain">{param}</div>', true);
		$this->addBBCode("pre", "<pre>{param}</pre>");
		$this->addBBCode("small", "<span class='small'>{param}</span>");
		$this->addBBCode("blur", "<span class='blur'>{param}</span>");
		$this->addBBCode("s", "<span style='text-decoration: line-through;'>{param}</span>");
		$this->addBBCode("o", "<span style='text-decoration: overline;'>{param}</span>");
		$this->addBBCode("reverse", "<span style='unicode-bidi:bidi-override;direction:rtl;'>{param}</span>");
		$this->addBBCode("flip", "<span class='fliptext'>{param}</span>");
		$this->addBBCode("h1", "<h1>{param}</h1>");
		$this->addBBCode("h2", "<h2>{param}</h2>");
		$this->addBBCode("h3", "<h3>{param}</h3>");
		$this->addBBCode("h4", "<h4>{param}</h4>");
		$this->addBBCode("h5", "<h5>{param}</h5>");
		$this->addBBCode("h6", "<h6>{param}</h6>");
		$this->addBBCode("hr", "<hr width='20%'>{param}");
		$this->addBBCode("php", "<a href='http://php.net/{param}' target='_new'>{param}()</a>");
		$this->addBBCode("psn", '<img src="http://pid.eu.playstation.com/user/{param}.jpg" width="235" height="149" border="0" />');
		$this->addBBCode("xblgt", '<iframe src="http://gamercard.xbox.com/{param}.card" scrolling="no" frameBorder="0" height="140" width="204">{param}</iframe>');
		$this->addBBCode("wow", "<img src='http://sigs.enjin.com/sig-wow/{param}.png' alt='WoW Sig from Enjin' />");
		$this->addBBCode("youtube", "<iframe width='440' height='350' src='http://www.youtube.com/embed/{param}' frameborder='0' allowfullscreen autostart='false'></iframe>");
		// $this->addBBCode("img", "<img src='{param}' title='Image' alt='Image' />");
