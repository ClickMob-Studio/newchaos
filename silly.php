...
if (mysql_num_rows($resultcheck) > 0) {
    ...
    if (($currenttime >= $worked911['starts']) && ($currenttime <= $worked911['ends'])) { $infected_mobster = true; }
        else { $infected_mobster = false; }

    // Admin Stuff

    if ($_POST['del_avatar'] && $user_class->accesslevel == 1) {
        if (file_exists('avatars/'.$profile_class->id.'.jpg')) { unlink('avatars/'.$profile_class->id.'.jpg'); }
            elseif (file_exists('avatars/'.$profile_class->id.'.gif')) { unlink('avatars/'.$profile_class->id.'.gif'); }
                elseif (file_exists('avatars/'.$profile_class->id.'.png')) { unlink('avatars/'.$profile_class->id.'.png'); }
    }
    ...
    if ($_GET['action'] == "addignore") {
        ...
        if ($numcontacts == 0) {
          if ($profile_class->id != "1") {
              ...
              } else { echo Message("You can't ignore yourself!"); include 'footer.php'; exit; }
          } else { echo Message("You can't ignore the KING!"); include 'footer.php'; exit; }
        }
    }

    if (file_exists('avatars/'.$profile_class->id.'.jpg')) { $avatar = 'avatars/' . $profile_class->id . '.jpg'; }
       elseif (file_exists('avatars/'.$profile_class->id.'.gif')) { $avatar = 'avatars/' . $profile_class->id . '.gif'; }
              elseif (file_exists('avatars/'.$profile_class->id.'.png')) { $avatar = 'avatars/' . $profile_class->id . '.png'; }
                     else { $avatar = 'avatars/0.jpg'; };
?>
<script type="text/javascript" src="https://worldofmobsters.com/includes/wz_tooltip.js"></script>
<div class="contenthead">Profile: <?php echo $profile_class->gamename ?> [<?php echo $profile_class->id; ?>]</div><!--contenthead-->
<div class="contentcontent">
		...
		<tr>
			<td colspan='4'>
				...
				<tr>
					<td width='120' align='center'><img height="100" width="100" src="<?php echo $profile_class->avatar; ?>"><?php echo $del_avatar; ?></td>
					<td align='center'>
                    <b>Status:</b> <?php echo $profile_class->mobsterstatus; ?><br><br>
<?php

if ($profile_class->id > 1) {
    if ($profile_class->scammer == "YES") { $profile_class->scammer = "<b><font color=red>YES</font></b>"; }
?>
                    <b>Rating:</b> <?php echo number_format($profile_class->rating); ?> [<a href="profiles.php?id=<?php echo $profile_class->id; ?>&rate=up">up</a> &bull; <a href="profiles.php?id=<?php echo $profile_class->id; ?>&rate=down">down</a>] &nbsp;&nbsp; <b>Scammer:</b> <?php echo $profile_class->scammer;?>
<?php

    if ($profile_class->pmban == 1) {
?>
                    <br><i><font color=red>User BANNED from the mailing system.</font></i>
<?php
...
?>
                    ...
                    <td align='right' valign='bottom'><img src='images/class<?php echo $profile_class->charclassid; ?>.jpg' width='70' height='70' alt='<?php echo $profile_class->charclass; ?>' title='<?php echo $profile_class->charclass; ?>'></td>
				</tr>
				</table>
			</td>
		</tr>
		...
		<tr>
			...
			<td width='35%'><?php echo $profile_class->formattedname; ?> [<b onmouseover="TagToTip('formH')" onmouseout="UnTip()">?</b>]&nbsp;</td>
		...
		<tr>
			...
			<td><b>House</b>:</td>
			<td><?php if ($profile_class->housename != "Homeless") { ?><a href="housedesc.php?id=<?php echo $profile_class->id; ?>"><?php echo $profile_class->housename; ?></a></td>
                                               <?} else { echo $profile_class->housename; ?></td><?}?>
		</tr>

<?php

if ($profile_class->hospital > time()) {
    if ((floor($profile_class->hospital - time()) / 60) != 1) { $plural = "s"; }
     $inhospital = 1;
...
if ($profile_class->jail > time()) {
    if ((floor($profile_class->jail - time()) / 60) != 1) { $plural = "s"; }
     $injail = 1;
...
//        $getusername = $mysql->query("SELECT `gamename` FROM `grpgusers` WHERE `id`='".$chkifreferred['referrer']."'");
//        $getuser = mysql_fetch_array($getusername);
        $wasreffered = $chkifreferred['gamename'];
        }
   else { $wasreffered = "Nobody"; }
...
?>
		...
		<table width='100%'>
			<tr>
				<td width='25%' align='center'><a href='pms.php?action=new&to=<?php echo $profile_class->id ?>'>Message</a></td>
				<td width='25%' align='center'><a href='attack.php?attack=<?php echo $profile_class->id ?>'>Attack</a></td>
				<td width='25%' align='center'><a href='mug.php?mug=<?php echo $profile_class->id ?>&r=<?php echo $newcode; ?>'>Mug</a></td>
			...
			<tr>
                ...
                <td width='25%' align='center'><a href='sendmulti.php?id=<?php echo $profile_class->id ?>'>Send Items</a></td>
                <td width='25%' align='center'><a href='sendcredit.php?person=<?php echo $profile_class->id ?>'>Send Credits</a></td>
            </tr>

			<tr>
                <td width='25%' align='center'><?php if ($is_friend) { ?> <a href='contactlist.php?action=friends&remove=<?php echo $profile_class->id ?>'>Remove Friend</a> <?php } else { ?><a href='profiles.php?action=addfriend&id=<?php echo $profile_class->id ?>'>Add Friend</a> <?php } ?></td>
                <td width='25%' align='center'><?php if ($is_enemy) { ?> <a href='contactlist.php?action=enemies&remove=<?php echo $profile_class->id ?>'>Remove Enemy</a> <?php } else { ?><a href='profiles.php?action=addenemy&id=<?php echo $profile_class->id ?>'>Add Enemy</a> <?php } ?></td>
				<td width='25%' align='center'><?php if ($is_ignored) { ?> <a href='contactlist.php?action=ignore&remove=<?php echo $profile_class->id ?>'>Remove Ignore</a> <?php } else { ?><a href='profiles.php?action=addignore&id=<?php echo $profile_class->id; ?>'>Add Ignore</a> <?php } ?></td>
                <td width='25%' align='center'><a href='hitman.php?userid=<?php echo $profile_class->id; ?>'>Add Bounty</a></td>
            </tr>
<?php
...
?>
                <td width='25%' align='center'><?php if ((($user_class->gangleader == $user_class->id) || ($rankinvites == 1)) && ($profile_class->gang == 0)) { ?> <a href='gang.php?action=invite&userid=<?php echo $profile_class->id ?>'>Invite to Gang</a> <?php } ?></td>
...
?>
        ...
        <div class="contentcontent">
            <table width='100%' id='cleanTable'>
                <tr>
                    <td width='15%'>IP:</td>
                    <td width='35%'><a href='search_ip.php?ip=<?php echo str_replace(".","-",$profile_class->ip); ?>'><?php echo $profile_class->ip; ?></a></td>
                ...
                <tr>
                    ...
                    <td>
                        <select name='flagtype'>
                            <option value='0'<?php if ($profile_class->scammer == "no") { echo " selected"; } ?>>No Flag</option>
                            <option value='1'<?php if ($profile_class->scammer == "<b><font color=red>YES</font></b>") { echo " selected"; } ?>>Scammer</option>
                        </select>
                    </td>
        ...
        <div class="contentcontent">
            ...
            function t_department( $number ) {
                if( $number == 1 ) $department = "Support";
                    elseif ( $number == 2 ) $department = "Bug Reports";
                        elseif ( $number == 3 ) $department = "Sales";
                return $department;
            }
            ...
            while($an_row = mysql_fetch_assoc($an_query))
            {
                ...
                if (($an_row['status'] == 'Pending') || ($an_row['status'] == 'Open')) {$an_row['status'] = "<font color='green'>".$an_row['status']."</font>";}
                if ($an_row['status'] == 'Responded') {$an_row['status'] = "<font color='yellow'>".$an_row['status']."</font>";}
                if ($an_row['status'] == 'Closed') {$an_row['status'] = "<font color='gray'>".$an_row['status']."</font>";}
                echo "<tr>
