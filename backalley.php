...
if ($user_class->hospital > 0) {
    ...
    $meds = '11, 12, 13, 14';
    $db->query("SELECT * FROM inventory LEFT JOIN items ON inventory.itemid = items.id WHERE itemid IN ($meds) AND userid = ?", array($user_class->id));

    $meds = $db->fetchAll(); // Assuming fetchAll() retrieves all rows matching the query. Adjust if your method name differs.
...
?>
		...
		<div class="floaty">
			...
			&bull; <font color="white">If you fail you will find yourself in the hospital</font><br /><br />
