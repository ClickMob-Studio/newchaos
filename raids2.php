...
mysql_query($query);
        ...
        $deduct_token_query = "UPDATE grpgusers SET raidtokens = raidtokens - $tokencost, raidshosted = raidshosted + 1,  raidsjoined = raidsjoined + 1 WHERE id = $user_id";
        mysql_query($deduct_token_query);
...
$user_id = $user_class->id; // Replace with the actual way to get the logged-in user's ID
    ...
    if ($raid['raid_type'] === 'Private' && $user_id != $raid['summoned_by']) {
        $raidTypeColor = 'red';
        $canJoin = false;
    } elseif ($raid['raid_type'] === 'Gang' && $user_gang_id != $raid_details['gang']) {
        $raidTypeColor = 'red';
        $canJoin = false;
    }
    ...
    echo $raid['id'];
    ?>
    <button onclick="fetchParticipants(<?= $raid['id']; ?>"'>View Participants</button>
<?php
}
   // First, get the boss ID for the raid to find out the max raiders
$boss_id_query = "SELECT boss_id FROM active_raids WHERE id = " . $raid['id'];
$boss_id_result = mysql_query($boss_id_query);
$boss_id_row = mysql_fetch_assoc($boss_id_result);
$boss_id = $boss_id_row['boss_id'];

// Now get the maxraiders value for this boss
$maxraiders_query = "SELECT maxraiders FROM bosses WHERE id = $boss_id";
$maxraiders_result = mysql_query($maxraiders_query);
$maxraiders_row = mysql_fetch_assoc($maxraiders_result);
$maxraiders = $maxraiders_row['maxraiders'];

// Then, get the number of current participants in the raid
...
// Finally, display the number of participants and the max raiders
if ($participants >= $maxraiders) {
    echo "<p style='color: red;'>Participants: " . $participants . "/" . $maxraiders . "[FULL]</p>";
} else {
    echo "<p>Participants: " . $participants . "/" . $maxraiders . "</p>";
}    
    echo "</div>";
}
echo "</div>";
...
<p><font color=red>Raids are intense battles where you summon and face off against formidable bosses. The higher your level, the mightier the bosses you can summon. Each boss brings unique challenges and lucrative rewards. Choose your difficulty wisely and team up with others to defeat these menacing creatures!</font></p>



<h2><center><font color=green>Available Bosses</font></center></h2>
...
foreach ($bosses as $boss) {
  ...
  echo "<form action='raids.php2' method='post' class='difficulty-form' onsubmit='return confirmSummon(" . $tokencost . ");'>";
