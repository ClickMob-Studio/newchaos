...
$db->query("UPDATE grpgusers SET fbitime = 0 WHERE id = ?");
           ...
           case 163:
              $db->query("UPDATE grpgusers SET bustpill = bustpill + 60 WHERE id = ?");
                ...
                break;
            case 253:
                $db->query("UPDATE user_ba_stats SET gold_rush_credits = gold_rush_credits + 10 WHERE user_id = ?");
                $db->execute(array(
                    $user_class->id
                ));
            ...
            case 258:
                ...
                echo Message("You open your loot crate and inside find 1.5m points, 5 x Double EXPs, 5 x Crime Boosters, 2 x Nerve Vials!");
                break;
