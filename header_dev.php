...
if (isset($_GET['spend'])) {
    if ($_GET['spend'] == "refenergy") {
		manual_refill('e');
        ($_SERVER['HTTP_REFERER']) ? header('Location: ' . $_SERVER['HTTP_REFERER']) : header('Location: https://themafialife.com/');
    }
    ...
    if ($_GET['spend'] == "refnerve") {
        ...
        elseif ($_SERVER['HTTP_REFERER'])
            ...
            header('Location: https://themafialife.com/');
    }
}
$browser = getBrowser();
...
function callback($buffer) {
    ...
    if (!$votes = $m->get('votes.' . $user_class->id)) {
		...
		$m->set('votes.' . $user_class->id, $votes, 5);
	}
...
if (empty($metatitle)) $metatitle = 'TrueMMO';
...
echo '<html>';
    echo '<head>';
        ...
        echo '<link href="https://fonts.googleapis.com/css?family=Chewy|Concert+One|Boogaloo|Germania+One" rel="stylesheet">';
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $.get("notiupdates.php", function (result) {
					...
					if(results[2] > 0)
						document.title = "(" + results[2] + ") TrueMMO";
                });
            }, 5000);
            });
        </script>
		...
		echo '<div id="topContent">';
			echo '<div id="outer" class="wrap">';
				...
				echo '<div id="main" class="row">';
					...
					echo '</div>';
                    echo '<div id="info">';
                        echo '<span class="page_resource">Food <span class="food">[' . $user_class->food . ']</span></span>';
                        echo '<span class="page_resource">Iron <span class="iron">[' . $user_class->iron . ']</span></span>';
                        echo '<span class="page_resource">Stone <span class="stone">[' . $user_class->stone . ']</span></span>';
                        echo '<span class="page_resource">Wood <span class="wood">[' . $user_class->wood . ']</span></span>';
		...
		echo '<div class="middle row">';
			echo '<div id="left">';
				echo '<div id="menu" class="sortMenu">';
                        ...
                        $gangURL = ($user_class->gang) ? 'gang.php' : 'creategang.php';
                            ...
                            $menus = array(
                                ...
                                22 => array(
                                    'title' => '<span class="<!_-votes-_!>">Vote for TMMO</span>',
                                    'url'   => 'vote.php'
                                ),
                                23 => array(
                                    'title' => 'My Pet',
                            ...
                            $db->query("SELECT menuorder FROM grpgusers WHERE id = ?");
                            $db->execute(array(
                                $user_class->id
                            ));

                            $row = $db->fetch_single();
                            $menuorder = explode(',', $row);
                            ...
                            if (!mysql_num_rows($result)) {
                                ...
                                <li class="slide">
                                    <div class="slide-content">
                                        <span>No messages at the moment! Click the icon on the left to post one!</span>
                                    </div>
                                </li>
                                <?php
                            } else {                                                             
                ...
                //if ($user_class->id == 150) {
                    ...
                    // if ($user_class->id == 150) {
                        // if (pathinfo($_SERVER['PHP_SELF'], PATHINFO_FILENAME) != 'patricks') {
                        //     echo '<div class="container">';

                        //     echo '<div class="progress">
                        //         <div class="progress-bar-heart">Shamrock</div>
                        //     </div>
                        // </div>';
                        // }
                    //}
                        ...
                        // echo '<script>
                        // var countDownDate = new Date("Mar 20, 2019 00:00:00").getTime();
                        ...
                        //   $(".progress-bar-heart").html("Shamrock 2x Speed Boost Active - " + minutes + "m " + seconds + "s ");

                        //   if (distance < 0) {
                        //     clearInterval(x);
                        //      $(".progress-bar-heart").html("Shamrock");
                        //   }
                        // }, 1000);
                        // </script>';
                //}
