...
<div class='box_middle'>
    <div class='pad'>
        ...
        if ($user_class->jail > 0)
            diefun("You cant go Raid Token smuggling while in prison.");
        if ($user_class->hospital > 0)
            diefun("You cant go Raid Token smuggling while in hospital.");
        if ($user_class->rtsmuggling == 0)
            diefun("You have smuggled as many Raid Tokens as you can today.");
        ...
        if (isset($_GET['smug'])) {
            if (!in_array($_GET['smug'], $places))
                ...
                $stole = rand(1, 7);
                $db->query("UPDATE grpgusers SET raidtokens = raidtokens + ?, rtsmuggling = rtsmuggling - 1 WHERE id = ?");
                $db->execute(array(
                    $stole,
                    $user_class->id
                ));

                $resultMessage = '<div style="text-align: center; padding: 15px; border: 1px solid #ccc; box-shadow: 3px 3px 10px rgba(0, 0, 0, 0.1); font-size: 15px; display: flex; align-items: center; justify-content: center;">You went to ' . $_GET['smug'] . ' and stole ' . $stole . ' Raid Tokens.</div><br>';
