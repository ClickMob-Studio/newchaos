...
$tbl_name  = "poker_poker";	//your table name
...
while ($tabler = $tableq->fetch(PDO::FETCH_ASSOC))
{
	...
	if($sbamount != 0) {
	$sb = money_small($sbamount, $tabler['gamestyle']);
	$bb = money_small($bbamount, $tabler['gamestyle']);
	} else {
	if ($tabletype == 't')
		{
	...
	$tableButtons  = '';
	$tableButtons .= $addons->get_hooks(
		array(
			'content' => ''
		),
		array(
	    	'page'     => 'admin/tables.php',
	    	'location'  => 'table_buttons'
		)
	);
	$opsTheme->addVariable('gametable_buttons', $tableButtons);
...
$gameStyles = $addons->get_hooks(
	array(
		'content' => array(
			...
			array('value' => 'p', 'label' => GAME_TEXASPOINTS),
