<?php
$players  = '';
$playersQ = $pdo->query("SELECT username FROM grpgusers");
...
$totalQ = $pdo->query("SELECT COUNT(ID) AS num FROM poker_sitelog {$whereSql}");
...
$logView = '';
$logs    = $pdo->query("SELECT * FROM poker_sitelog {$whereSql} ORDER BY ID DESC LIMIT $start, $limit");
