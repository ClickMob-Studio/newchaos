    ...
    function fillboxes($curr){
        ...
        $rows = $db->fetch_row();
        foreach($rows as $row){
            $rtn .= '<tr>'; // Each bet is its own row
            $rtn .= '<td>'. formatName($row['userid']) .'</td>';
            $rtn .= '<td>'. prettynum($row['amnt'], ($curr == 'cash' ? 1: 0)) .'</td>';
            $btn = ($user_class->id == $row['userid']) ? 'Remove Bet' : 'Take Bet';
            $rtn .= '<td><button onclick="'. (($user_class->id == $row['userid']) ? "takeaway" : "take") .'(' . $row['id'] . ');">'. $btn .'</button></td>';
            $rtn .= '</tr>';
        }
        return $rtn;
    }
