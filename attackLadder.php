...
function checkSpotTaken($spot) {
    $query = sprintf("SELECT COUNT(*) as count FROM `attackladder` WHERE `spot` = %d", mysql_real_escape_string($spot));
...
function deleteUserFromSpot($userId, $spot) {
    $deleteQuery = sprintf("DELETE FROM `attackladder` WHERE `user` = %d AND `spot` = %d", mysql_real_escape_string($userId), mysql_real_escape_string($spot));
...
if (!checkSpotTaken($desiredSpot)) {
    $insertQuery = sprintf("INSERT INTO `attackladder` (`user`, `spot`) VALUES (%d, %d)", mysql_real_escape_string($userId), mysql_real_escape_string($desiredSpot));
