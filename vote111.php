...
$sites = array(
   ...
   "mpogtop" => "http://mpogtop.com/in/1691346692",
    "top100arena" => "http://www.top100arena.com/in.asp?id=98581",
    "topgamesites" => "http://www.topgamesites.net/mmorpg",
    "xtremetop" => "http://www.xtremetop100.com/in.php?site=1132371964",
	"gtop100" => "https://gtop100.com/topsites/MMORPG-And-MPOG/sitedetails/TheMafiaLife-100023?vote=1",
...
if (isset($_GET['vote']) && array_key_exists($_GET['vote'], $sites)) {
	...
	$db->query("UPDATE grpgusers SET points = points + 200, votetokens = votetokens + 10, money = money + 50000 WHERE id = ?");
	$db->execute(array(
		$user_class->id
	));
...
echo'<div class="floaty" style="margin:5px;width:75%;>';
	echo'<span style="color:red;font-weight:bold;">Vote Tokens Availible: <span style="color:#00FFFF;"> ' . prettynum($user_class->votetokens) . ' Vote Points</span></br></span>';

	echo'<span style="color:red;font-weight:bold;">Each vote gives you: <span style="color:#00FFFF;">200 Points</span> and <span style="color:green;">$50,000</span> + <span style="color:#00FFFF;">10 Vote Points</span></BR></BR></span>';
...
foreach ($sites as $name => $link) {
	...
	echo'<div class="floaty flexcont" style="margin:5px;width:75%;padding:0;' . $opc .'">';
		...
		echo'<div class="flexele">';
			echo'<span style="color:#00FFFF;">Votes Today: ' . $vts . '</span><br />';
		echo'</div>';
	echo'</div>';
	}
include 'footer.php';
