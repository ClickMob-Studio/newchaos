<?php
include 'header.php';
$sites = array(
    "mpogtop" => "http://mpogtop.com/in/1527615688",
	"gtop100" => "http://www.gtop100.com/topsites/MMORPG-And-MPOG/sitedetails/Another-Asylum-94322?vote=1",
    "top100arena" => "http://www.top100arena.com/in.asp?id=93878",
    "topgamesites" => "http://www.topgamesites.net/mmorpg",
    "xtremetop" => "http://www.xtremetop100.com/in.php?site=1132365635",
);
if (isset($_GET['vote']) && array_key_exists($_GET['vote'], $sites)) {
	$db->query("SELECT * FROM votes WHERE userid = ? AND site = ?");
	$db->execute(array(
		$user_class->id,
		$_GET['vote']
	));
	$voted = $db->fetch_row(true);
    if (!empty($voted))
        diefun("You have already voted there for today!");
	$db->query("INSERT INtO votes VALUES ('', ?, ?)");
	$db->execute(array(
		$user_class->id,
		$_GET['vote']
	));
	$db->query("UPDATE grpgusers SET points = points + 250, money = money + 10000 WHERE id = ?");
	$db->execute(array(
		$user_class->id
	));
    header('location: ' . $sites[$_GET['vote']]);
}
echo'<div class="floaty" style="margin:5px;width:75%;>';
	echo'<span style="color:red;font-weight:bold;">Each vote gives you: <span style="color:#00FFFF;">250 Points</span> and <span style="color:green;">$10,000</span></span>';
echo'</div>';
foreach ($sites as $name => $link) {
	$db->query("SELECT * FROM votes WHERE userid = ? AND site = ?");
	$db->execute(array(
		$user_class->id,
		$name
	));
	if($db->fetch_row(true)){
		$opc = "background:rgba(51, 51, 51, .75);opacity:.5;";
		$vt = 'Yes!';
		$color = 'green';
	}else{
		$opc = "";
		$vt = 'No?';
		$color = 'red';
	}
	$db->query("SELECT COUNT(*) FROM votes WHERE site = ?");
	$db->execute(array(
		$name
	));
	$vts = $db->fetch_single();
	echo'<div class="floaty flexcont" style="margin:5px;width:75%;padding:0;' . $opc .'">';
		echo'<div class="flexele" style="border-right:thin solid #333;line-height:30px;">';
			echo'<a href="?vote=' . $name . '">';
				echo'<span style="color:red;font-weight:bold;">' . $name . '</span>';
			echo'</a>';
		echo'</div>';
		echo'<div class="flexele" style="border-right:thin solid #333;line-height:30px;">';
			echo'<span style="color:' . $color . ';">Did you vote? ' . $vt . '</span>';
		echo'</div>';
		echo'<div class="flexele">';
			echo'<span style="color:#00FFFF;">Votes Today: ' . $vts . '</span><br />';
		echo'</div>';
	echo'</div>';
	}
include 'footer.php';
