        ...
        <div class='pad'>
            ...
            if (isset($_GET['buy'])) {
                ...
                if ($_GET['buy'] == "points_six") {
                    ...
                    if ($user_class->credits >= $cost) {
                        $newcredit = $user_class->credits -= $cost;
                        $db->query("UPDATE grpgusers SET points = points + 600000, credits = credits - ? WHERE id = ?");
                        $db->execute(array(
                            $cost,
                            $user_class->id
                        ));
                        echo Message("You spent " . $cost . " credits for 600000 Points.");
                        Send_Event(2, $user_class->formattedname ." bought 600000 points");
                        Send_Event(1, $user_class->formattedname ." bought 600000 points");
                    } else {
                        echo Message("You don't have enough credits. You can buy some at the upgrade store.");
                    }
                }
                ...
                if ($_GET['buy'] == "points_seven") {
                    ...
                    if ($user_class->credits >= $cost) {
                        $newcredit = $user_class->credits -= $cost;
                        $db->query("UPDATE grpgusers SET points = points + 1000000, credits = credits - ? WHERE id = ?");
                        $db->execute(array(
                            $cost,
                            $user_class->id
                        ));
                        echo Message("You spent " . $cost . " credits for 1000000 Points.");

                        Send_Event(2, $user_class->formattedname ." bought 1000000 points");
                        Send_Event(1, $user_class->formattedname ." bought 1000000 points");
