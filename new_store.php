        ...
        if (isset($_GET['buy'])) {
            ...
            if ($_GET['buy'] == "freebie") {
                if ($user_class->claimed == 1) {
                    ...
                    perform_query("UPDATE `grpgusers` SET `claimed` = 1 WHERE id = ?", [$user_class->id]);
                    perform_query("UPDATE grpgusers SET  points = points + 25000, raidtokens = raidtokens + 50, claimed = 1, credits = credits - 0 WHERE id = ?", [$user_class->id]);
