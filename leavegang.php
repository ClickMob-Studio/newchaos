...
if ($_GET['x'] == "leave") {
    if ($user_class->weploaned == 1) {
        ...
        mysql_query("UPDATE grpgusers SET eqweapon = 0, weploaned = 0 WHERE id = $user_class->id");
    }
    if ($user_class->armorloaned == 1) {
        AddToArmory($user_class->eqarmor, $user_class->gang);
        mysql_query("UPDATE grpgusers SET eqarmor = 0, armloaned = 0 WHERE id = $user_class->id");
    }
    if ($user_class->shoesloaned == 1) {
        AddToArmory($user_class->eqshoes, $user_class->gang);
        mysql_query("UPDATE grpgusers SET eqshoes = 0, shoeloaned = 0 WHERE id = $user_class->id");
    }
    $result = mysql_query("SELECT * FROM gang_loans WHERE to = $user_class->id");
    while ($line = mysql_fetch_array($result)) {
        AddToArmory($line['item'], $user_class->gang);
        mysql_query("DELETE FROM gang_loans WHERE id = {$line['id']}");
    }
