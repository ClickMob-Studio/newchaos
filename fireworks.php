...
//         $result = mysql_query("UPDATE `offers` SET `remaining` = `remaining` - 1 WHERE `id` != '" . $row['id'] . "'");
...
//         Send_Event($user_class->id, "You have been credited your Limited Shamrock Double!.", $user_class->id);
//         $db->execute(array());
//         echo Message("You spent 100 credits on your Limited Shamrock Double.");
//     } else {
...
<div class="floaty">
    <h2 class="text-center mb-0" style="color:#7eff11">Independence Day Event</h2>
    <h2 class="text-14 m-0">28th - 12July  2022 (Ends 23:59 12th)</h2>
    <p class="text-14">Earn <?php echo item_popup('Fireworks', 158, '#7eff11') ?> based on your activity level</p>
        <img src='https://themafialife.com/css/images/NewGameImages/fireworks.png' width="100px">
    <div style="text-align:left;padding:10px;">
        <p class="text-14"><?php echo item_popup('Fireworks', 158, '#7eff11') ?> will appear in your inventory and you can share with another player - you will both receive a random reward (same for both).</p>
        ...
        <p class="text-14">So the more active you are the more <?php echo item_popup('Fireworks', 158, '#7eff11') ?> you will earn - make sure to share those fireworks!!</p>
        <!-- <h2 class="text-center mb-0">Who will you share your <?php echo item_popup('Fireworks', 158, '#7eff11') ?> with?</h2> -->
    </div>
    ...
    <div class="package" style="width:50%;margin:10px auto;border:1px white solid;padding:10px;">
        ...
        <p class="text-14 m-0"><?php echo item_popup('Fireworks', 158, '#7eff11') ?> <strong>[x3]</strong> <br/>
            Cost: 100 Credits<br/>
    ...
    <?php
        $db->query("SELECT count(*) as total, `to` FROM `eventslog` WHERE text = 'You have been awarded Fireworks! Goto your Inventory and celebrate the 4th of July with your friends!' GROUP BY `to` ORDER BY total DESC");
        $db->execute();
        ...
        <h2>Statistics</h2>
        <h2 class="mb-0" style="color:#7eff11">Most Fireworks Earned</h2>
        ...
        foreach ($results as $result) {
            ...
            //echo '</td><td style="color:#' . $colors[$i] . ';font-weight:700;">' . item_popup('Fireworks', 158, '#7eff11') . ' [x' . $rewards[$i] . ']</td>';
            $i++;
        }
