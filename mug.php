        ...
        if ($mug <= 8) {
            ...
            if ($success) {
                ...
                if ($mugamount < 1) {
                    ...
                    perform_query("UPDATE grpgusers SET moth = moth + 1, motd = motd + 1 WHERE id = ?", [$user_class->id]);
                    $toadd = array('motd' => 1);
                    ...
                    if ($user_class->gang != 0 && $gang_class->tax > 0) {
                        ...
                        perform_query("UPDATE `gangs` SET `moneyvault` = ? WHERE `id` = ?", [$newvault, $user_class->gang]);
                    }
                    ...
                    perform_query("UPDATE grpgusers SET money = ?, muggedmoney = ?, mugsucceeded = ?, moth = moth + 1, motd = motd + 1, tamt = tamt + ? WHERE id = ?", [$newmuggeramount, $muggedmoneygain, $mugsucceeded, $mugamount, $user_class->id]);
                    perform_query("UPDATE grpgusers SET muggedmoney = ?, money = ? WHERE id = ?", [$muggedmoneylost, $newmuggedamount1, $attack_person->id]);
                    $online = (time() - $attack_person->lastactive < 900) ? 1 : 0;
                    perform_query("INSERT INTO muglog (mugger, mugged, amount, active, timestamp) VALUES(?, ?, ?, ?, unix_timestamp())", [$user_class->id, $attack_person->id, $mugamount, $online]);
                    mission('m');
                    newmissions('mugs');
                    updateGangActiveMission('mugs', 1);
                    gangContest(array('mugs' => 1));
            ...
            $timee = 300;
            perform_query("UPDATE `grpgusers` SET `jail` = ? WHERE `id` = ?", [$timee, $user_class->id]);
        }
        $newnerve = $user_class->nerve - 10;
        perform_query("UPDATE `grpgusers` SET `nerve` = ?, `last_mug_time` = ? WHERE `id` = ?", [$newnerve, time(), $user_class->id]);
