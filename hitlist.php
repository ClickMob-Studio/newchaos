      ...
      //  if ($ref == "" || $ref != "https://mafialords.com/hitlist.php") {
       ...
       // }
        ...
        if ($error != "")
            ...
            $resultm = mysql_query("UPDATE grpgusers SET bank = '" . $newmoney . "' WHERE id = '" . $user_class->id . "'");
            mysql_query("INSERT INTO bank_log VALUES('', $user_class->id, {$bounty}, 'mwith', $user_class->bank, unix_timestamp())");
            $result = mysql_query("INSERT INTO hitlist (target, reason, bounty, `from`) VALUES ('$target', '$reason', '$bounty', '" . $user_class->id . "')");
    ...
    if (isset($_GET['remove'])) {
        ...
        if (!empty($error))
            ...
            $resultm = mysql_query("UPDATE grpgusers SET bank = $newmoney WHERE id = $user_class->id");
            mysql_query("INSERT INTO bank_log VALUES('', $user_class->id, {$worked['bounty']}, 'mdep', $user_class->bank, unix_timestamp())");
            $delete = mysql_query("DELETE FROM hitlist WHERE id = {$_GET['remove']}");
    ...
    $error = ($attack_person->city != $user_class->city && $user_class->id != 146) ? "You must be in the same city as the person you're attacking!" : $error;
        ...
        if (isset($error)) {
            ...
            if ($theirhp <= 0) {
                ...
                $newmoney = $user_class->bank + $worked['bounty'];
                mission('k');
                $result = mysql_query("UPDATE grpgusers SET bank = $newmoney WHERE id = $user_class->id");
                mysql_query("INSERT INTO bank_log VALUES('', $user_class->id, $worked[bounty], 'mdep', $user_class->bank, unix_timestamp())");
                ...
                $delete = mysql_query("DELETE FROM hitlist WHERE id = {$_GET['hit']}");
            }
            if ($yourhp <= 0) {
                $winner = $attack_person->id;
                $result = mysql_query("UPDATE grpgusers SET hwho = $attack_person->id, hhow = 'attacked', hwhen = '" . date(g . ":" . i . ":" . sa, time()) . "', hospital = 300 WHERE id = $user_class->id");
                $yourhp = 0;
