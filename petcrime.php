    ...
    <div class='pad'>
        ...
        if (!empty($_GET['id'])) {
            ...
            elseif ($chance <= $successchance) {
                ...
                perform_query("UPDATE pets SET jail = ($jailtime*60), nerve = ? WHERE userid = ?", [$pet_class->nerve, $user_class->id]);
                ...
                perform_query("UPDATE pets SET nerve = nerve - ? WHERE userid = ?", [$row['nerve'], $user_class->id]);
