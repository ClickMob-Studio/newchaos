    ...
    if ($_GET['buy'] == "limitedpack") {
        if ($user_class->limiteditems1 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems1` = limiteditems1 - 1 WHERE `limiteditems1` != '0'");
                         $db->query("UPDATE grpgusers SET  credits = credits - 200, points = points + 25000 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Limited Pointrs  Pack!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 200 credits on your Points PACK. This has been added to your bank!");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "limitedpack2") {
        if ($user_class->limiteditems2 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems2` = limiteditems2 - 1 WHERE `limiteditems2` != '0'");
                         $db->query("UPDATE grpgusers SET  points = points + 250000, bank = bank + 0, credits = credits - 2000 WHERE id = ?");
  ...
  Give_Item(9, $user_class->id, 5);
            ...
            Send_Event($user_class->id, "You have been credited your Private War Package!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 2,000 credits on your Private War Package.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "limitedpack3") {
        if ($user_class->limiteditems3 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems3` = limiteditems3 - 1 WHERE `limiteditems3` != '0'");
                         $db->query("UPDATE grpgusers SET  points = points + 1850000, credits = credits - 10000 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Ultra War Package!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 10,000 credits on your Ultra War Package.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "freebie") {
        if ($user_class->claimed == 1) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `claimed` = 1 WHERE id = ?");
                         $db->query("UPDATE grpgusers SET  points = points + 15000, claimed = 1, credits = credits - 0 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Freebie NewYear Package!.", $user_class->id);
            $db->execute(array());
            echo Message("Happy New Year! Lets Start 2022 with a bang everyone. You have been credited 15,000 Points!.");
        } else {
            echo Message("You have already claimed your New Year Package");
        }
    }
