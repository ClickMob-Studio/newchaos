...
<div class="floaty">
    <h2 class="text-center mb-0" style="color:#7eff11">Activity Event</h2>
    <h2 class="text-14 m-0">13th - 25th July 2023 (Ends 23:59 35th)</h2>
    <p class="text-14">Earn <?php echo item_popup('Snowball/s', 198, '#7eff11') ?> based on your activity level</p>
        <img src='https://s2.themafialife.com/images/snowball.png' width="100px">
    <div style="text-align:left;padding:10px;">
        <p class="text-14"><?php echo item_popup('Snowball/s', 198, '#7eff11') ?> will appear in your inventory and you can share with another player - you will both receive a random reward (same for both).</p>
        ...
        <p class="text-14">So the more active you are the more <?php echo item_popup('Snowball/s', 198, '#7eff11') ?> you will earn - make sure to share the love!</p>
        <!-- <h2 class="text-center mb-0">Who will you share your <?php echo item_popup('Snowball/s', 198, '#7eff11') ?> with?</h2> -->
    </div>

    <!-- <h2 class="text-center m-0"  style="color:#7eff11">Can't wait? Need More Rayz's?</h2>
    <div class="package" style="width:50%;margin:10px auto;border:1px white solid;padding:10px;">
        <p class="text-18 mt-0">Activity Double - Limited Offer #2</p>
        <p class="text-14 m-0"><?php //echo item_popup('Snowball/s', 198, '#7eff11') ?> <strong>[x3]</strong> <br/>
            Cost: 100 Credits<br/>
    ...
    <?php
        $db->query("SELECT count(*) as total, `to` FROM `eventslog` WHERE text = 'You have been awarded a Snowball! Throw these at other players!' AND timesent > 1689861600 AND timesent <= 1703577599 GROUP BY `to` ORDER BY total DESC");
        $db->execute();
        ...
        foreach ($results as $result) {
            ...
            echo '</td><td style="color:#' . $colors[$i] . ';font-weight:700;">' . item_popup('Snowballs', 198, '#7eff11') . ' [x' . $rewards[$i] . ']</td>';
            $i++;
        }
        ...
        echo '<h2 class="mb-0" style="margin-top: 10px;color:#7eff11">Most Snowballed Player</h2>
        ...
        if (!empty($results)) {
            foreach ($results as $result) {
                ...
                echo '</td><td style="color:#' . $colors[$i] . ';font-weight:700;">' . item_popup('Snowball/s', 198, '#7eff11') . ' [x' . $rewards[$i] . ']</td>';
                $i++;
            }
