...
if (isset($_GET['confirm_buy'])) {
    ...
    if ($user['money'] < $business['cost']) {
        ...
        mysql_query("UPDATE grpgusers SET money = money - {$business['cost']} WHERE id = '$user_id'");

        // Insert the purchase into the ownedbusinesses table with the custom name
        mysql_query("INSERT INTO ownedbusinesses (user_id, business_id, purchase_date, status, name, rating, employees, intelligence, cost) VALUES ('$user_id', '$business_id', NOW(), 'Active', '$custom_name', '{$business['rating']}', '{$business['employees']}', '{$business['intelligence']}', '{$business['cost']}')");
        
        // Fetch the ownership_id of the last inserted business
        $ownership_id = mysql_insert_id();
        mysql_query("UPDATE grpgusers SET current_employer = '$ownership_id' WHERE id = '$user_id'");
