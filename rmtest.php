    ...
    if ($_GET['buy'] == "limitedpack") {
        if ($user_class->limiteditems1 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems1` = limiteditems1 - 1 WHERE `limiteditems1` != '0'");
                         $db->query("UPDATE grpgusers SET  credits = credits - 100, points = points + 30000 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Standard New Years Package!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 100 credits on your Standard New Years Package!");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "limitedpack2") {
        if ($user_class->limiteditems2 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems2` = limiteditems2 - 1 WHERE `limiteditems2` != '0'");
                         $db->query("UPDATE grpgusers SET  points = points + 350000, bank = bank + 0, credits = credits - 1000 WHERE id = ?");
 ...
 Give_Item(42, $user_class->id, 10);
            ...
            Send_Event($user_class->id, "You have been credited your Large New Years Pack!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 1000 credits on your Large New Years Pack.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "limitedpack3") {
        if ($user_class->limiteditems3 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems3` = limiteditems3 - 1 WHERE `limiteditems3` != '0'");
                         $db->query("UPDATE grpgusers SET  points = points + 5000000, credits = credits - 10000 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Mega New Years Package!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 10,000 credits on your Mega New Years Package.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "freebie") {
        if ($user_class->claimed == 1) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `claimed` = 1 WHERE id = ?");
                         $db->query("UPDATE grpgusers SET  points = points + 5000, christmasraffle = christmasraffle + 10, claimed = 1, credits = credits - 0 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Free Package 5,000 Points & a Double Exp Pill!. You also Receieved 10 Raffle Tickets", $user_class->id);
            $db->execute(array());
            echo Message("Happy Levelling!. You have been credited 5,000 Points & a Double Exp Pill You also Receieved 10 Raffle Tickets.");
        } else {
...
echo '<h2>Make a Donation to TheMafiaLife</h2><br/>You can earn 1 Christmas Rafle Ticket for every $1 you spend! You can also obtain tickets through Christmas Presents!';
echo '<h2><font color=red>Christmas Rafle Tickets : [' . $user_class->christmasraffle . ']</h2></font>';
