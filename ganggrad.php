...
function buy() {
    ...
    if ($gang_class->formattedTag == "Yes")
        ...
        $insert = mysql_query("update gangs set pointsvault = pointsvault - " . $removeAmnt . ", formattedTag = 'Yes' where id = " . $gang_class->id) or die(mysql_error());
...
function submit() {
    ...
    if (!preg_match($pattern, $first) || !preg_match($pattern, $second) || !preg_match($pattern, $third))
        diefun("There was an error.", "Please try again
        <input type='submit' value='Back' onclick=location.href='ganggrad.php' />");
    $insert = mysql_query("update gangs set tColor1 = '" . $first . "', tColor2 = '" . $second . "', tColor3 = '" . $third . "' where id = " . $gang_class->id) or die(mysql_error());
    if ($insert)
