...
<table align="center" width=100%>
  <tr><td class="contenthead"><b>Crack the Safe</b></td></tr> 
<?php
...
$code = rand(0,149);
       ...
       mysql_query("UPDATE `grpgusers` SET `money`=`money` + 7000000 WHERE `id`='".$_SESSION['id']."'");      
                     mysql_query("UPDATE `[kraakkluis]` SET `kluis`='5000',`code`='$code', `winnaar`='$user_class->username' WHERE `area`='2'");
 mysql_query("DELETE FROM `[kraaknr]` WHERE `area`='2'");
print"<tr><td class=\"contentcontent\">You cracked the code!";
}
else{
            $data->belcredit			- 10;
           mysql_query("UPDATE `grpgusers` SET `points`=`points`-5, `crack`=`crack` + 1 WHERE `id`='".$_SESSION['id']."'");
            mysql_query("UPDATE `[kraakkluis]` SET `kluis`=`kluis`+'10' WHERE `area`='2'");
    mysql_query("INSERT INTO `[kraaknr]` (time,login,getal,area) VALUES (NOW(),'$user_class->username', '$code','2')");
