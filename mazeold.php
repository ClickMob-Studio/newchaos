...
$query2 = "SELECT * FROM user_logs ORDER BY user_id = $user_class->id DESC LIMIT 10";
...
function updateLogs() {
    $.ajax({
        ...
        success: function(data) {
            console.log("Logs data:", data); // Debugging line
            $('.user-logs').html(data); // This assumes fetch_logs.php returns the <table>...</table>
        },
        complete: function() {
...
$(document).ready(function() {
    ...
    updateLogs(); // Kick off the first update
});

function sendDirection(chosenDirection) {
    // Hide previous feedback (results and turns left)
    document.querySelector("#searchFeedback").style.display = "none";

    // Show the spinner in place of the results
    document.querySelector(".spinner").style.display = "block";

    // Use setTimeout to wait for 1 second
    setTimeout(function() {
        // Existing logic for sending direction
        console.log("Button clicked with direction:", chosenDirection);
        ...
        fetch('process_maze_event.php', {
            method: 'POST',
            body: formData, // Use 'body' to send FormData
        })
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            // Show the feedback (results and turns left)
