...
if (isset($_GET['bail'])) {
    ...
    mysql_query("UPDATE grpgusers SET points = points - 5 WHERE id = $user_class->id");
    mysql_query("UPDATE pets SET jail = 0 WHERE userid = $user_class->id");
...
if (!empty($_GET['jailbreak'])) {
    ...
    $chance = rand(1, 100);
    if ($chance <= 92) {
        mysql_query("UPDATE pets SET exp = exp + 500, busts = busts + 1, nerve = nerve - 5 WHERE id = $pet_class->id");
        mysql_query("UPDATE pets SET jail = 0 WHERE userid = {$_GET['jailbreak']}");
        ...
        mysql_query("UPDATE pets SET jail = jail + 300, nerve = nerve - 5 WHERE id = $pet_class->id");
        ...
        mysql_query("UPDATE pets SET nerve = nerve - 10 WHERE id = $pet_class->id");
