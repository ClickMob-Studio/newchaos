...
if ($user_class->polled1active == 1) {
    if ($user_class->polled1 == 0) {
        if (isset($_POST['submit'])) {
            if ($_POST['poll1'] != "") {
                $result = mysql_query("UPDATE `grpgusers` SET `polled1` = '1', `points` = points+20 WHERE `id` = '" . $user_class->id . "'");
                $result = mysql_query("UPDATE `poll1` SET `votes` = votes+1 WHERE `optionid` = '" . $_POST['poll1'] . "'");
        ...
        <tr valign="top"><div class="contentcontent">
            ...
            <form method="post">
                <?php
                $result = mysql_query("SELECT * FROM `poll1` ORDER BY `optionid` ASC");
