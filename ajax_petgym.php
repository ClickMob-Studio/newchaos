...
if (isset($_POST['what']) AND $_POST['what'] == 'refill') {
    ...
    if ($att == 'energy') {
        ...
        elseif ($user_class->points < 10)
            ...
            mysql_query("UPDATE pets SET energy = $pet_class->maxenergy WHERE userid = $user_class->id");
        ...
        elseif ($user_class->points < $ptstouse)
            ...
            mysql_query("UPDATE grpgusers SET points = points - $ptstouse WHERE id = $user_class->id");
            mysql_query("UPDATE pets SET awake = $pet_class->maxawake WHERE userid = $user_class->id");
        ...
        elseif ($pet_class->energy == $pet_class->maxenergy)
            ...
            $pet_class->energypercent = floor(($pet_class->energy / $pet_class->maxenergy) * 100);
            $pet_class->formattedenergy = $pet_class->energy . " / " . $pet_class->maxenergy . " [" . $pet_class->energypercent . "%]";
            mysql_query("UPDATE pets SET energy = $pet_class->maxenergy, awake = $pet_class->maxawake WHERE userid = $user_class->id");
            mysql_query("UPDATE grpgusers SET points = points - $ptstouse WHERE id = $user_class->id");
