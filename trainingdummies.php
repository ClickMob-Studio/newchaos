...
if (isset($_GET['attack']) && (int)$_GET['attack'] && (int)$_GET['attack'] > 0) {
    ...
    if ($bossHp <= 0) {
        ...
        $db->query('UPDATE training_dummy_user SET level = level + 1, exp + ?, last_fight_time = ? WHERE id = ?');
        $db->execute(array($newExp, time(), $trainingDummyUserToUse['id']));
