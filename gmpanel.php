...
require_once 'gmheader.php';
require_once 'includes/class_mtg_paginate.php';
...
if (isset($_GET['action']) && $_GET['action'] == 'acthourtest') {
    view_act_hour_logs_2();
    include 'footer.php';
    die();
}
$pages = new Paginator();
...
if (isset($_GET['page_action'])) {
    ...
    if ($_GET['page_action'] == "maillog") {
		...
		if($user_class->id == 9)
			$sql = ' WHERE `to` <> 9 AND `from` <> 9';
		else
			$sql = ' WHERE `to` <> 21 AND `from` <> 21';
    ...
    if ($_GET['page_action'] == "5050check") {
        ...
        $table = (isset($_GET['pts'])) ? "pts5050log" : "fiftyfiftylogs";
        ...
        <table id='newtables' style='width:100%;'>
            ...
            if (!$db->num_rows())
                ...
                foreach ($rows as $row) {
                    ...
                    $matcher = new User($row['taker']);
                    $winner = formatName($row['winner']);
                    //$winner = $row['better'] == $row['winner'] ? formatName($row['better']) : formatName($row['winner']);
                    $amnt = isset($_GET['pts']) ? prettynum($row['amnt']) . " points" : prettynum($row['amnt'], 1);
...
function view_act_hour_logs_2() {
    $file = '/home/mafialor/public_html/actlog.txt';
    $current = file_get_contents($file);
    preg_match_all("/([0-9]*)\|-\|-\|([^\|]*)\|-\|-\|(.+)\|-\|-\|([0-9]*);/", $current, $out, 1);
    print_r($out);
}
...
<br />
    <form method='get' action='gmpanel.php'>
        ...
        <input type='text' name='action' value='acthour'>
...
<table width=100% cellspacing='1' class='table'> \n<tr style='background:rgba(0,255,0,.33)'> <th>Username</th> <th>Page</th> <th>POST TAGS</th> <th>Time</th> <th>Last</th> </tr>";
    ...
    $hour = date('G', $time) - 1;
    if (isset($_GET['time'])) {
        $file = "/home/mafialor/public_html/actlog.txt" . $month . "_" . $day . "_" . $hour . $file;
    } else {
        $file = '/home/mafialor/public_html/actlog.txt';
    }
    $current = file_get_contents($file);
    ...
    for ($i = $_GET['st']; $i < count($out[1]); $i++) {
        ...
        if (contains($out[2][$i], $ignore))
            continue;

        // if ($out[2][$i] == '/gmpanel.php?action=acthour')
        //     continue;
