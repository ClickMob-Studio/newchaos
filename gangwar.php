...
if ($user_class->gang != 0) {
    ...
    if (isset($_POST['invite'])) {
        ...
        if ($cash > 5000000)
            ...
            $invited = new Gang($to);
            $result = mysql_query("INSERT INTO gangwars (gang1, gang2, bet, timesent) VALUES ($user_class->gang, $to, $cash, unix_timestamp())");
            $gang->moneyvault -= $cash;
            $result = mysql_query("UPDATE gangs SET moneyvault = $gang->moneyvault WHERE id = $user_class->gang");
        ...
        if ($war == 0) {
            ...
            if ($check > 0) {
                ...
                if ($warnumber['bet'] <= $gang->moneyvault) {
                    ...
                    mysql_query("UPDATE gangs SET moneyvault = $gang->moneyvault WHERE id = $user_class->gang");
    ...
    if (isset($_GET['decline'])) {
        ...
        if ($war == 0) {
            ...
            if ($check > 0) {
                ...
                $result = mysql_query("DELETE FROM gangwars WHERE warid = {$_GET['decline']} AND gang2 = $user_class->gang LIMIT 1");
                $theirgang = new Gang($warnumber['gang1']);
                $theirgang->moneyvault += $warnumber['bet'];
                $result = mysql_query("UPDATE gangs SET moneyvault = $theirgang->moneyvault WHERE id = $theirgang->id");
