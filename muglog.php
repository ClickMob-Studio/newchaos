...
<tr><td class='contentcontent'>
        <table width="100%" id="attacklog">
            ...
            echo "</table>";
            $count = mysql_fetch_array(mysql_query("SELECT count(*) from muglog WHERE mugger = $user_class->id OR mugged = $user_class->id"));
            $count = (($count['count'] / 10) > 30) ? 30 : ($count['count'] / 10);
            for ($i = 1; $i <= $count; $i++) {
                if ($i == 1)
                    print "Pages: ";
                if ($i == $_GET['page'])
