...
register_shutdown_function(function () {
    ...
    if ($e && in_array($e['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        error_log("Minute cron FATAL: {$e['message']} in {$e['file']}:{$e['line']}\n", 3, __DIR__ . '/cron_minute.log');
    }
});
function logc($m)
{
...
try {
    ...
    foreach ($raids as $raid) {
        ...
        if (empty($participants) || $total_strength <= 0) {
            $raid_successful = false;
            $battle_log .= "No eligible participants or zero strength; raid failed.\n";
            $boss_hp = 0; // prevent loop
        } else {
            $max_rounds = 10000;
            $round = 0;
            while ($boss_hp > 0 && $round < $max_rounds) {
                foreach ($participants as $key => &$participant) {
                    // HP is ignored in this version
                    $user_hp = 100000000;

                    // Determine weapon
                    $weapon = $participant['itemname'] ?: "fists";
                    ...
                    $damage_to_boss = round(mt_rand($base_damage - $damage_variation, $base_damage + $damage_variation));
                ...
                $round++;
                if ($round >= $max_rounds && $boss_hp > 0) {
                    $battle_log .= "Your party has exhausted all options; the boss survived.\n";
                    $raid_successful = false;
                }
            }
        }
        ...
        foreach ($participants as $participant) {
            ...
            if ($raid_successful) {
                ...
                $points_won = mt_rand($total_min_points, $total_max_points);
                $money_won = mt_rand($total_min_money, $total_max_money);
                $raidpoints_won = mt_rand($total_min_raidpoints, $total_max_raidpoints);  // New
                ...
                if (!empty($loot_table)) {
                    foreach ($loot_table as $loot) {
                        $random_chance = mt_rand(0, 100 - $raid_boost_statues); // Generate a number between 0 and 100

                        if ($raid['used_booster']) {
                            $random_chance = mt_rand(0, 80 - $raid_boost_statues);
                        }
                    ...
                    if ($participant['leashed_pet_id']) {
                        foreach ($loot_table as $loot) {
                            $random_chance = mt_rand(0, 100); // Generate a number between 0 and 100
...
awake = LEAST(
    ...
    $rand = mt_rand(1, 16); // Randomly select how many users to affect, between 1 to 3.
    ...
    if (!empty($bbinfo)) {
        ...
        $db->query("SELECT b.* FROM bbusers b JOIN grpgusers g ON g.id = b.userid WHERE g.admin = 0");
        $db->execute();
        $rows = $db->fetch_row() ?: [];
        foreach ($rows as $row) {
            $info[] = $row;
        ...
        if (!empty($info)) {
            perform_query("UPDATE bloodbath SET winners = ? WHERE id = ?", [serialize($info), $bbinfo['id']]);
            perform_query("INSERT INTO bloodbath (endtime, winners, is_paid) VALUES (unix_timestamp() + (86400*7), '', 0)");
            $db->query("TRUNCATE TABLE bbusers");
            $db->execute();
        } else {
            logc('bloodbath: no bbusers rows to serialize');
        }
    }
