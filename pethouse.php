...
if (isset($_GET['action']) && $_GET['action'] == 'sell') {
    ...
    mysql_query("UPDATE grpgusers SET money = money + $cost WHERE id = $user_class->id");
    mysql_query("UPDATE pets SET house = 0, awake = 0, maxawake = 100 WHERE userid = $user_class->id");
...
if (isset($_GET['buy'])) {
    ...
    mysql_query("UPDATE grpgusers SET money = money - $cost WHERE id = $user_class->id");
    mysql_query("UPDATE pets SET house = {$_GET['buy']}, awake = {$new['awake']}, maxawake = {$new['awake']} WHERE userid = $user_class->id");
