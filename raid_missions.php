...
include "header.php";
?>
<div class='box_top'>Raid Missions</div>
<div class='box_middle'>
    <div class='pad'>
        ...
        if (isset($_GET['reset_mission']) && (int) $_GET['reset_mission']) {
            ...
            perform_query("UPDATE missions SET completed = 'no', timestamp = ?, crimes = 0, mugs = 0, kills = 0, busts = 0, backalleys = 0, crimes_paid = 0, busts_paid = 0, kills_paid = 0, mugs_paid = 0 WHERE id = ?", [$now, $r['id']]);
        ...
        if (isset($_GET['do'])) {
            ...
            if ($q)
                ...
                if ($mm['between'] + $r['timestamp'] > $currenttime) {
                    ...
                    perform_query("INSERT INTO missionlog (`id`, `text`, `timestamp`) VALUES(?, ?, ?)", ['[x] started a ' . $mm['name'] . ',' . $user_class->id, time()]);
                }
        ...
        $q2 = mysql_query("SELECT * FROM mission WHERE category = 7 ORDER BY id ASC ");
...
<div class=\"hundred centered\">";
        ...
        if (mysql_fetch_array($check)) {
            ...
            $raids = ($miss['raids'] > $usermission['raids']) ? "<font color='red'>{$usermission['raids']}/{$miss['raids']}</font>" : "<font color='green'>{$miss['raids']}/{$miss['raids']}</font>";
            $currenttime = time();
...
<tr>
    ...
    <tr>
        <td>Raids</td>
        <td>Complete Raids</td>
        <td>{$miss['payRaids']} points</td>
        <td>{$raids}</td>
    ...
    <table width='100%' style='background-color: #292929; border-collapse: collapse;'>
        <thead>
            ...
            <tr>
                ...
                <td class='mission-columns'>Kills: <span class='text-green'>{$v['kills']}<br /></span>Crimes: <span class='text-green'>{$v['crimes']}<br /></span>Mugs: <span class='text-green'>{$v['mugs']}<br /></span>Busts: <span class='text-green'>{$v['busts']}</span><br />Backalleys: <span class='text-green'>{$v['backalleys']}</span><br />Raids: <span class='text-green'>{$v['raids']}</span></td>
                <td class='mission-columns'>Kills: <span class='text-green'>{$v['payKills']}</span> Points<br/>Crimes: <span class='text-green'>{$v['payCrimes']}</span> Points<br/>Mugs: <span class='text-green'>{$v['payMugs']}</span> Points<br/>Busts: <span class='text-green'>{$v['payBusts']}</span> Points<br/>Backalleys: <span class='text-green'>{$v['payBackalleys']}</span> Points<br/>Raids: <span class='text-green'>{$v['payRaids']}</span> Points<br/>EXP: <span class='text-green'>{$v['exp_level']}%</span> of max EXP<br/></td>
