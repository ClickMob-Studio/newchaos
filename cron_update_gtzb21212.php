    ...
    foreach ($gangTerritoryZoneBattles as $gangTerritoryZoneBattle) {
        ...
        if ($battleInitiationTime < time()) {
            ...
            if ($attackingHealth <= 0) {
                ...
                $db->query("UPDATE gang_territory_zone_battle SET winning_gang_id = " . $defendingGang->id . ", attacking_total_stats = " . $totalAttackingStats . ", defending_total_stats = " . $totalDefendingStats . ", is_complete = 1  WHERE id = " . $gangTerritoryZone['id']);
                $db->execute();
            } else {
                // Attacking Team Won
