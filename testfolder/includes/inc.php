...
if(isset($cap) && empty($m->get('captcha.'.$user_class->id))){
...
if ($user_class->gang) {
    if (!$m->get('gangTM.' . $user_class->gang)) {
        $db->query("SELECT strength, defense, speed FROM grpgusers WHERE gang = ?");
        $db->execute(array(
            $user_class->gang
        ));
        $rows = $db->fetch_row();
        $total = 0;
        foreach ($rows as $row)
            $total += $row['strength'] + $row['defense'] + $row['speed'];
        $db->query("UPDATE gangs SET tmstats = ? WHERE id = ?");
        $db->execute(array(
            $total,
            $user_class->gang
        ));
        $m->set('gangTM.' . $user_class->gang, 'set', 300);
    }
}
if ($user_class->bf) {
...
function callback($buffer) {
    global $user_class, $db, $m, $includepets;
    if (!$m->get('5seccache')) {
        ...
        $referrals = $info['refs'];
        $tickets = $info['tick'];
        $m->set('hitlist', $hitlist);
        $m->set('animals', $animals);
        $m->set('refs', $referrals);
        $m->set('tick', $tickets);
        $m->set('4seccache', 'set', 5);
    } else {
        $hitlist = $m->get('hitlist');
        $animals = $m->get('animals');
        $referrals = $m->get('refs');
        $tickets = $m->get('tick');
    }
    if (!$m->get('mail.' . $user_class->id)) {
        $db->query("SELECT COUNT(id) FROM pms WHERE `to` = ? AND viewed = 1 AND deleted = 0");
        $db->execute(array(
            $user_class->id
        ));
        $mail = $db->fetch_single();
        $m->set('mail.' . $user_class->id, $mail, 1);
    } else
        $mail = $m->get('mail.' . $user_class->id);
    if (!$m->get('events.' . $user_class->id)) {
        $db->query("SELECT COUNT(id) FROM events WHERE `to` = ? AND viewed = 1");
        $db->execute(array(
            $user_class->id
        ));
        $events = $db->fetch_single();
        $m->set('events.' . $user_class->id, $events, 5);
    } else
        $events = $m->get('events.' . $user_class->id);
    if (!$m->get('hospital')) {
        $db->query("SELECT COUNT(id) FROM grpgusers WHERE hospital > 0");
        $db->execute();
        $hospital = $db->fetch_single();
        $m->set('hospital', $hospital, 60);
    } else
...
if (!$m->get('lastnews')) {
    ...
    $m->set('lastnews', $lastpost, 3600);
} else
    $lastpost = $m->get('lastnews');
            ...
            <tr>
                <td colspan="2" class="pos1" valign="top">
                    <div class="topbox">
                        <table>
                            <tr>
                                <td colspan='2' width="99%" style="font-weight:700;text-align:left;vertical-align:top;padding:0;" class="resize">
                                    <div style="background:rgba(0,0,0,.75);width:200px;height:150px;margin:0;padding-left:25px;">
                                        <div style="margin:0 auto;white-space:nowrap;line-height:18.5px;">
                                            ...
                                            ?></span><br />
                                            &nbsp;<strong>Money: </strong> <span style='color:gold;font-weight:700;' class='money'>$<!_-money-_!></span> <a href='bank.php?dep'><img src='images/deposit.gif' title='Deposit' alt='Deposit'/></a></span> <a href='sendmoney.php'><img src='images/send.jpg' title='Send' alt='Send'/></a><br />
                                            &nbsp;<strong>Points: </strong> <a href='spendpoints.php' style='color:white;font-weight:700;'><!_-points-_!></a> <a href='sendpoints.php'><img src='images/send.jpg' title='Send' alt='Send'/></a><br />
                                            &nbsp;<strong>Credits: </strong> <a href='rmstore.php' style='color:yellow;'><!_-credits-_!></a> <a href='sendcredits.php'><img src='images/send.jpg' title='Send' alt='Send'/></a>
                                        </div>
                                    </div>
                                </td>
                    ...
                    <table style="width:100%;border-spacing:0px;margin-top:5px;">
                        <tr>
                            <td valign="top" width="175">
                                ...
                                <a class="leftmenu" href="index.php"><img src="images/point.png" width="16" height="16" border="0"/> Home</a>
                                <a class="leftmenu" href="dailies.php"><img src="images/point.png" width="16" height="16" border="0"/> Check Dailies</a>
                                <a class="leftmenu" href="tavern.php"><img src="images/point.png" width="16" height="16" border="0"/> <!_-GCLINK-_!></a>
                                <a class="leftmenu" href="inventory.php"><img src="images/point.png" width="16" height="16" border="0"/> Inventory</a>
                                <a class="leftmenu" href="city.php" style="font-weight:700;"><img src="images/point.png" width="16" height="16" border="0"/> <!_-cityname-_!> </a>
                                <a class="leftmenu" href="bank.php"><img src="images/point.png" width="16" height="16" border="0"/> Bank</a>
                                <a class="leftmenu" href="gym.php"><img src="images/point.png" width="16" height="16" border="0"/> Gym </a>
                                <a class="leftmenu" href="<!_-crimepage-_!>.php"><img src="images/point.png" width="16" height="16" border="0"/> Crime</a>
                                ...
                                <a class="leftmenu" href="gangmail.php"><img src="images/point.png" width="16" height="16" border="0"/> Gang Mail [<!_-gmail-_!>]</a>
                                <a class="leftmenu" href="pms.php?view=inbox"><img src="images/point.png" width="16" height="16" border="0"/> Mailbox [<span class='mail'><!_-mail-_!></span>]</a>
                                <a class="leftmenu" href="events.php"><img src="images/point.png" width="16" height="16" border="0"/> Events [<span class='events'><!_-events-_!></span>]</a>
                                <a class="leftmenu" href="jail.php"><img src="images/point.png" width="16" height="16" border="0"/> Jail <span class='inmates'><!_-jail-_!></span></a>
                                <a class="leftmenu" href="hospital.php"><img src="images/point.png" width="16" height="16" border="0"/> Hospital <!_-hospital-_!></a>
                                <a class="leftmenu" href="petjail.php"><img src="images/point.png" width="16" height="16" border="0"/> Dog Pound <span class='animals'><!_-jail2-_!></span></a>
                                <a class="leftmenu" href="hitlist.php"><img src="images/point.png" width="16" height="16" border="0"/> Hitlist [<span class='hitlist'><!_-hitlist-_!></span>]</a>
                                <a class="leftmenu" href="missions.php"><img src="images/point.png" width="16" height="16" border="0"/> Missions [<!_-missions-_!>]</a>
                                <a class="leftmenu" href="backalley.php"><img src="images/point.png" width="16" height="16" border="0"/> <span style="color:yellow;">Back Alley</span></a>
                                <a class="leftmenu" href="smartads.php"><img src="images/point.png" width="16" height="16" border="0"/> Smart Ads</a>
                                <a class="leftmenu" href="spylog.php"><img src="images/point.png" width="16" height="16" border="0"/> Spy Log</a>
                                <a class="leftmenu" href="notepad.php"><img src="images/point.png" width="16" height="16" border="0"/> Notepad</a>
                                <a class="leftmenu" href="search.php"><img src="images/point.png" width="16" height="16" border="0"/> Thug Search</a>
                                <a class="leftmenu" href="classifieds.php"><img src="images/point.png" width="16" height="16" border="0"/> Newspaper</a>
                                <a class="leftmenu" href="chat.php"><img src="images/point.png" width="16" height="16" border="0"/> Thug Chat <!_-CHAT_USERS-_!></a>
                                <a class="leftmenu" href="forum.php"><img src="images/point.png" width="16" height="16" border="0"/> Forums</a>
                                <a class="leftmenu" href="index.php?action=logout" style="font-weight:700;"><img src="images/point.png" width="16" height="16" border="0"/> Logout</a><?php
                                if ($user_class->gm) {
                                    ?><div class="space"></div>
                                    <div class="contenthead">Staff Panel</div>
                                    <a class="leftmenu" href="gmpanel.php"><img src="images/point.png" width="16" height="16" border="0"/> GM Panel</a>
                                    <a class="leftmenu" href="managetickets.php"><img src="images/point.png" width="16" height="16" border="0"/> Support Desk [<!_-tickets-_!>]</a><?php
                                }
                                if ($user_class->fm) {
                                    ?><div class="space"></div>
                                    <div class="contenthead">Staff Panel</div>
                                    <a class="leftmenu" href="fmpanel.php"><img src="images/point.png" width="16" height="16" border="0"/> FM Panel</a><?php
                                }
                                if ($user_class->admin) {
                                    ?><div class="space"></div>
                                    <div class="contenthead">Staff Panel</div>
                                    <a class="leftmenu" href="gmpanel.php"><img src="images/point.png" width="16" height="16" border="0"/> GM Panel</a>
                                    <a class="leftmenu" href="fmpanel.php"><img src="images/point.png" width="16" height="16" border="0"/> FM Panel</a>
                                    <a class="leftmenu" href="managetickets.php"><img src="images/point.png" width="16" height="16" border="0"/> Support Desk [<!_-tickets-_!>]</a>
                                    <div class="space"></div>
                                    <div class="contenthead">Admin Panel</div>
                                    <a class="leftmenu" href="staff_rtstore.php"><img src="images/point.png" width="16" height="16" border="0"/> Manage RTStore</a>
                                    <a class="leftmenu" href="staff_missions.php"><img src="images/point.png" width="16" height="16" border="0"/> Manage Missions</a>
                                    <a class="leftmenu" href="control.php?page=referrals"><img src="images/point.png" width="16" height="16" border="0"/> Referrals [<!_-referrals-_!>]</a>
                                    <a class="leftmenu" href="paypal_log.php"><img src="images/point.png" width="16" height="16" border="0"/> Paypal Logs</a>
                                    <a class="leftmenu" href="gmpanel.php?page_action=maillog"><img src="images/point.png" width="16" height="16" border="0"/> Mail log</a>
                                    <a class="leftmenu" href="statcounts.php"><img src="images/point.png" width="16" height="16" border="0"/> Stat Counts</a>
                                    <a class="leftmenu" href="control.php"><img src="images/point.png" width="16" height="16" border="0"/> Admin Stuff</a>
                                    <a class="leftmenu" href="editfeatures.php"><img src="images/point.png" width="16" height="16" border="0"/> Edit crime/house</a>
                                    <a class="leftmenu" href="massmail.php"><img src="images/point.png" width="16" height="16" border="0"/> Mass Mail</a>
                                    <a class="leftmenu" href="staff_mail.php"><img src="images/point.png" width="16" height="16" border="0"/> Mass Emailer</a><?php
                                }
                                ?><div class="space"></div>
                                <div class="contenthead">Pet</div>
                                <a class="leftmenu" href="pethof.php"><img src="images/point.png" width="16" height="16" border="0"/> Pet Hall Of Fame</a>
                                <a class="leftmenu" href="petshop.php"><img src="images/point.png" width="16" height="16" border="0"/> Pet Shop</a>
                                <a class="leftmenu" href="petgym.php"><img src="images/point.png" width="16" height="16" border="0"/> Pet Gym</a>
                                <a class="leftmenu" href="petcrime.php"><img src="images/point.png" width="16" height="16" border="0"/> Pet Crime</a>
                                <a class="leftmenu" href="pettrack.php"><img src="images/point.png" width="16" height="16" border="0"/> <span style="color:yellow;">Pet Track</span></a>
                                <a class="leftmenu" href="mypets.php"><img src="images/point.png" width="16" height="16" border="0"/> My Pets</a>
                                <a class="leftmenu" href="petspylog.php"><img src="images/point.png" width="16" height="16" border="0"/> Pet Spy Log</a>
                                <div class="space"></div>
                                <div class="contenthead">Support</div>
                                <a class="leftmenu" href="tickets.php"><img src="images/point.png" width="16" height="16" border="0"/> Support Desk</a>
                                <a class="leftmenu" href="bbcode.php"><img src="images/point.png" width="16" height="16" border="0"/> BBCode Help</a>
                                <a class="leftmenu" href="forum.php?id=8"><img src="images/point.png" width="16" height="16" border="0"/> Help Forum</a>
                                <div class="space"></div>
                                <div class="contenthead">Account</div>
                                <a class="leftmenu" href="preferences.php"><img src="images/point.png" width="16" height="16" border="0"/> Edit Account</a>
                                <a class="leftmenu" href="cpassword.php"><img src="images/point.png" width="16" height="16" border="0"/> Change Password</a>
                                <a class="leftmenu" href="contactlist.php"><img src="images/point.png" width="16" height="16" border="0"/> Contact List</a>
                                <a class="leftmenu" href="ignorelist.php"><img src="images/point.png" width="16" height="16" border="0"/> Ignore List</a>
                                <a class="leftmenu" href="rmstore.php"><img src="images/point.png" width="16" height="16" border="0"/> Upgrade Account</a>
                            </td>
                            <td valign="top">
                                <table style="width:100%;border-spacing:0px;">
                                    <tr>
                                        <td>
                                            <table class="content" style="border-spacing:0px;align:right;margin-left:3px;margin-top:-1px;">
                                                <?php
                                                ...
                                                if (isset($set['admin'])) {
                                                    ?>
                                                    <tr><td class='contenthead'>Admin Notification</td></tr>
                                                    <tr><td class='contentcontent center'><?php
                                                ...
                                                if (!$m->get('uni.' . $user_class->id)) {
                                                    ...
                                                    if ($db->num_rows())
                                                        if (time() >= $db->fetch_single())
                                                            $unifinished = 1;
                                                    $m->set('uni.' . $user_class->id, $unifinished, 300);
                                                } else
                                                    $unifinished = $m->get('uni.' . $user_class->id);
                                                if ($user_class->gang) {
                                                    if ($m->get('gangcrime.' . $user_class->gang) == null) {
                                                        $db->query("SELECT crime FROM gangs WHERE id = ?");
                                                        $db->execute(array(
                                                            $user_class->gang
                                                        ));
                                                        $crime = $db->fetch_single();
                                                        $m->set('gangcrime.' . $user_class->gang, $crime, 60);
                                                    } else
                                                        $crime = $m->get('gangcrime.' . $user_class->gang);
                                                    if ($crime) {
                                                    ...
                                                    if (!$m->get('gangwarCheck.' . $user_class->gang)) {
                                                        ...
                                                        if ($db->num_rows())
                                                            if (time() >= $db->fetch_single())
                                                                $warfinished = 1;
                                                        $m->set('gangwarCheck.' . $user_class->gang, $warfinished, 300);
                                                    } else
                                                        $warfinished = $m->get('gangwarCheck.' . $user_class->gang);
                                                } else {
                                                ...
                                                if (!$m->get('rel.' . $user_class->id)) {
                                                    ...
                                                    $m->set('rel.' . $user_class->id, $rtn, 120);
                                                } else
                                                    $rtn = $m->get('rel.' . $user_class->id);
                                                if ($rtn != "none")
                                                    echo Message($rtn);
