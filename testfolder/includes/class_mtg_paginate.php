...
class Paginator {
	...
	function paginate() {
		...
		if($_GET['ipp'] == 'All')
			...
			if(!ctype_digit($this->items_per_page))
			...
			for($i = 1; $i <= $this->num_pages; $i++) {
				...
				if($i == 1 Or $i == $this->num_pages Or in_array($i, $this->range)) {
					$this->return .= ($i == $this->current_page And $_GET['page'] != 'All')
						? "<a title='Go to page ".$i." of ".$this->num_pages."' class='page active' href='#'>".$i."</a> "
						: "<a class='page gradient' title='Go to page ".$i." of ".$this->num_pages."' href='".$_SERVER['PHP_SELF']."?page=".$i."&amp;ipp=".$this->items_per_page.$this->querystring."'>".$i."</a> ";
				}
				if($this->range[$this->mid_range - 1] < $this->num_pages - 1 And $i == $this->range[$this->mid_range - 1])
					$this->return .= " ... ";
			}
			$this->return .= (($this->current_page < $this->num_pages And $this->items_total >= 10) And ($_GET['page'] != 'All') And $this->current_page > 0)
				? "<a class='page gradient' href='".$_SERVER['PHP_SELF']."?page=".$next_page."&amp;ipp=".$this->items_per_page.$this->querystring."'>Next &raquo;</a>\n"
				: "<span class='page gradient'>&raquo; Next</span>\n";
			$this->return .= ($_GET['page'] == 'All')
				? "<a class='page active' style='margin-left:10px' href='#'>All</a> \n"
				: "<a class='page gradient' style='margin-left:10px' href='".$_SERVER['PHP_SELF']."?page=1&amp;ipp=All".$this->querystring."'>All</a> \n";
		} else {
			for($i = 1; $i <= $this->num_pages; $i++) {
				$this->return .= ($i == $this->current_page)
					? "<a class='page active' href='#'>".$i."</a> "
					: "<a class='page gradient' href='".$_SERVER['PHP_SELF']."?page=".$i."&amp;ipp=".$this->items_per_page.$this->querystring."'>".$i."</a> ";
			}
			$this->return .= "<a class='page gradient' href='".$_SERVER['PHP_SELF']."?page=1&amp;ipp=All".$this->querystring."'>All</a> \n";
		}
		$this->low = ($this->current_page <= 0) ? 0 : ($this->current_page - 1) * $this->items_per_page;
		if($this->current_page <= 0)
			$this->items_per_page = 0;
		$this->limit = ($_GET['ipp'] == 'All')
			? "" :
	...
	function display_items_per_page() {
		...
		return "<span class='page gradient'>Items per page:</span><select class='page gradient' onchange='window.location='".$_SERVER['PHP_SELF']."?page=1&amp;ipp='+this[this.selectedIndex].value+'".$this->querystring."';return false'>".$items."</select>\n";
	}
