       ...
       // echo 'Bloodbath will be paid automatically when the timer expires.</font></center><tr>';
        ...
        foreach ($loop as $lop => $prizes) {
            ...
            <table id="newtables" style="width:100%;table-layout:fixed;margin-top:20px">
                ...
                <?php
                $lol = str_replace(' ', '', $lop);
                $result = mysql_query("SELECT b.* FROM bbusers b LEFT JOIN grpgusers g ON userid = id WHERE b.$lol <> 0 AND lastactive > unix_timestamp() - (86400 * 7) AND g.admin = 0 ORDER BY b.$lol DESC LIMIT $nor");
                $rank = 0;
                $top = array();
                ...
                if($user_class->id == 682) {
                    if(!in_array($user_class->id, $top)) {
                        $position = mysql_query("SELECT userid, `$lol`, FIND_IN_SET( `$lol`, (
                            SELECT GROUP_CONCAT(DISTINCT `$lol`
                            ORDER BY `$lol` DESC )
                            FROM bbusers )
                            ) AS rank
                            FROM bbusers
                        ...
                        $ranking = mysql_fetch_array($position);
                        $rank = $ranking['rank'];
