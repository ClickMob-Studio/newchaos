    ...
    if ($_GET['buy'] == "limitedpack") {
        if ($user_class->limiteditems1 == 0) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems1` = limiteditems1 - 1 WHERE `limiteditems1` != '0'");
                         $db->query("UPDATE grpgusers SET  credits = credits - 200, points = points + 20000 WHERE id = ?");
            ...
            Give_Item(42, $user_class->id, 2);




            Send_Event($user_class->id, "You have been credited your Limited Points Pack!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 250 credits on your Points PACK.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "limitedpack2") {
        if ($user_class->limiteditems2 == 0) {
            echo Message("This Pack Is No Longer Available.");
        } else if ($user_class->credits >= 990) {
            $newcredit = $user_class->credits -= 990;
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems2` = limiteditems2 - 1 WHERE `limiteditems2` != '0'");
                         $db->query("UPDATE grpgusers SET  points = points + 125000, credits = credits - 990 WHERE id = ?");
            ...
            Send_Event($user_class->id, "You have been credited your Spring Pack!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 990 credits on your Spring Pack.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
if ($_GET['buy'] == "limitedpack3") {
        if ($user_class->limiteditems3 == 0) {
            echo Message("This Pack Is No Longer Available.");
        } else if ($user_class->credits >= 750) {
            ...
            $result    = mysql_query("UPDATE `grpgusers` SET `limiteditems3` = limiteditems3 - 1 WHERE `limiteditems3` != '0'");
                         $db->query("UPDATE grpgusers SET  points = points + 0, credits = credits - 750 WHERE id = ?");
 ...
 Give_Item(68, $user_class->id);
            ...
            Send_Event($user_class->id, "You have been credited your Limited Items Pack!.", $user_class->id);
            $db->execute(array());
            echo Message("You spent 750 credits on your Limited Pills PACK.");
        } else {
            echo Message("You don't have enough credits. You can buy some at the upgrade store.");
        }
    }
...
echo '<h2>Make a Donation to TrueMMO</h2>';
...
// echo '<hr>';
echo '<center><font size="3px"><font color=white>$1 = 10 Credits</font></center>';
...
echo '<input type="hidden" name="item_name" value="Donation to TrueMMO">';
...
<div class="section">
    ...
    <div class="section-items">
        ...
        <div class="new-shop-item has-tooltip">
...
<div class="section-row">
    <div class="section" style="flex-basis: 60%">
        <h4 class="section-title">RM Packs</h4>
        <div class="section-items">
...
echo '<h3>By donating to TrueMMO you are agreeing to the following terms:</h3>';
echo '<hr>';
...
echo '<li>If you don\'t get your package, please contact <a href="mailto:support@truemmo.com"><span style="color:red;">support@truemmo.com</span></a> with the paypal receipt number</li>';
