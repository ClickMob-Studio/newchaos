...
function get_ava($usr)
{
    ...
    $usrq = $pdo->prepare("select avatar from grpgusers where username = '" . $usr . "' ");
    $usrq->execute();
...
function display_ava($usr)
{
    ...
    $usrq->execute();
    $usrr = $usrq->fetch(PDO::FETCH_ASSOC);
    $avatar = '<img src="' . $usrr['avatar'] . '" border="0">';
    return $avatar;
}
...
function display_ava_profile($usr)
{
    ...
    $usrq->execute();
    $usrr = $usrq->fetch(PDO::FETCH_ASSOC);
    $avatar = '<img src="' . $usrr['avatar'] . '?x=' . $time . '" border="0">';
    return $avatar;
}
...
function poker_log($playername, $msg, $gameID)
{
    ...
    if (strlen($playername) > 0)
    {
        ...
        $plyrQ = $pdo->query("SELECT ID, avatar FROM grpgusers WHERE username = '$playername'");
