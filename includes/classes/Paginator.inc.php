<?php
    ...
    class Paginator
    {
        public static $recordsOnPage = 20;
        public static $scrollPages = 6;
        ...
        {
            return preg_replace('/' . $this->pageVar . '=[^&]*&?|^&/', '', $this->queryString);
        }
        ...
        */
        public function getFirstPageNav($user_link = '', $linkParam = '', $returnHtml = false)
        {
            ...
            if (!$this->isFirstPage() && $this->showFirstLast) {
                if (empty($this->jsHandler)) {
                    $html = '<a href="' . $this->pageName . '?' . $this->pageVar . '=0' . $this->queryString . '" ' . $linkParam . ' class="button">' . $user_link . '</a>';
                } else {
                    $html = '<a href="javascript:void(0);" onclick="' . $this->jsHandler . '(\'' . $this->pageName . '\',\'page=0' . $this->queryString . '\')" ' . $linkParam . ' class="button">' . $user_link . '</a>';
                }
            } elseif ($this->showFirstLast && $this->showDisabledLinks) {
                $html = '<a href="javascript:void(0);" class="button disabled">' . $user_link . '</a>';
            }
        ...
        public function getLastPageNav($user_link = '', $linkParam = '', $returnHtml = false)
        {
            ...
            if (!$this->isLastPage() && $this->showFirstLast) {
                if (empty($this->jsHandler)) {
                    $html = '<a href="' . $this->pageName . '?' . $this->pageVar . '=' . ($this->totalPage - 1) . $this->queryString . '" ' . $linkParam . ' class="button">' . $user_link . '</a>';
                } else {
                    $html = '<a href="javascript:void(0);" onclick="' . $this->jsHandler . '(\'' . $this->pageName . '\',\'' . $this->pageVar . ($this->totalPage - 1) . $this->queryString . '\')" ' . $linkParam . ' class="button">' . $user_link . '</a>';
                }
            } elseif ($this->showFirstLast && $this->showDisabledLinks) {
                $html = '<a href="javascript:void(0);" class="button disabled">' . $user_link . '</a>';
            }
        ...
        public function getNextPageNav($user_link = '', $linkParam = '', $returnHtml = false)
        {
            ...
            if (!$this->isLastPage() && $this->showPrevNext) {
                if (empty($this->jsHandler)) {
                    $html = '<a href="' . $this->pageName . '?' . $this->pageVar . '=' . ($this->page + 1) . $this->queryString . '" ' . $linkParam . ' class="button nextButton">' . $user_link . '</a>';
                } else {
                    $html = '<a href="javascript:void(0);" onclick="' . $this->jsHandler . '(\'' . $this->pageName . '\',\'' . $this->pageVar . ($this->page + 1) . $this->queryString . '\')" ' . $linkParam . ' class="button nextButton">' . $user_link . '</a>';
                }
            } elseif ($this->showPrevNext && $this->showDisabledLinks) {
                $html = '<a href="javascript:void(0);" class="button disabled nextButton">' . $user_link . '</a>';
            }
        ...
        public function getPrevPageNav($user_link = '', $linkParam = '', $returnHtml = false)
        {
            ...
            if (!$this->isFirstPage() && $this->showPrevNext) {
                if (empty($this->jsHandler)) {
                    $html = '<a href="' . $this->pageName . '?' . $this->pageVar . '=' . ($this->page - 1) . $this->queryString . '" ' . $linkParam . ' class="button prevButton">' . $user_link . '</a>';
                } else {
                    $html = '<a href="javascript:void(0);" onclick="' . $this->jsHandler . '(\'' . $this->pageName . '\',\'' . $this->pageVar . ($this->page - 1) . $this->queryString . '\')" ' . $linkParam . ' class="button prevButton">' . $user_link . '</a>';
                }
            } elseif ($this->showPrevNext && $this->showDisabledLinks) {
                $html = '<a href="javascript:void(0);" class="button disabled prevButton">' . $user_link . '</a>';
            }
        ...
        public function getScrollPrevPageNav($user_link = '', $linkParam = '', $returnHtml = false)
        {
            ...
            if ($this->page > $this->scrollPage && $this->showScrollPrevNext) {
                if (empty($this->jsHandler)) {
                    $html = '<a href="' . $this->pageName . '?' . $this->pageVar . '=' . ($this->page - $this->scrollPage) . $this->queryString . '" ' . $linkParam . '>' . $user_link . '</a>';
                } else {
                    $html = '<a href="javascript:void(0);" onclick="' . $this->jsHandler . '(\'' . $this->pageName . '\',\'' . $this->pageVar . ($this->page - $this->scrollPage) . $this->queryString . '\')" ' . $linkParam . '>' . $user_link . '</a>';
                }
        ...
        public function getNumberPageNav($linkParam = '', $returnHtml = false)
        {
            ...
            for (; $i < $j + $scrollPage && $i < $this->totalRecords; ++$i) {
                if ($i == $this->page) {
                    $html .= '<a href="javascript:void(0);" class="button disabled">' . ($i + 1) . '</a>';
                } else {
                    if (empty($this->jsHandler)) {
                        $html .= '<a href="' . $this->pageName . '?' . $this->pageVar . '=' . $i . $this->queryString . '" ' . $linkParam . ' class="button">' . ($i + 1) . '</a>';
                    } else {
                        $html .= '<a href="javascript:void(0);" onclick="' . $this->jsHandler . '(\'' . $this->pageName . '\',\'' . $this->pageVar . $i . $this->queryString . '\')" ' . $linkParam . ' class="button">' . ($i + 1) . '</a>';
                    }
                }
            }
        ...
        public function getPageNav($linkParam = '', $returnHtml = false)
        {
            ...
            $this->calculate();
            $html .= $this->getFirstPageNav('', $this->linkParam, $returnHtml);
            $html .= $this->getScrollPrevPageNav('', $this->linkParam, $returnHtml);
            $html .= $this->getPrevPageNav('', $this->linkParam, $returnHtml);
            $html .= $this->getNumberPageNav($this->linkParam, $returnHtml);
            $html .= $this->getNextPageNav('', $this->linkParam, $returnHtml);
            $html .= $this->getScrollNextPageNav('', $this->linkParam, $returnHtml);
            $html .= $this->getLastPageNav('', $this->linkParam, $returnHtml);
