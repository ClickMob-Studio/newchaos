...
class RPItem
{
    ...
    public function ShowPayment($method, User $user, array $options = [])
    {
        ...
        if ($method == 'moneybooker') {
            return Payment::RenderMoneyPayment('www.generalforces.com|DP|' . $this->id . '|' . $for, $this->item['code'], number_format($this->GetPrice($method), 2));
        } elseif ($method == 'paypal') {
            return Payment::RenderPaypalPayment('www.generalforces.com|DP|' . $this->id . '|' . $for, $this->item['name'], number_format($this->GetPrice($method), 2));
        } elseif ($method == 'pbc') {
            return Payment::RenderCashPayment($user->id, 'www.generalforces.com@DP@' . $this->id . '@' . $forPBC, number_format($this->GetPrice(), 2), $this->item['name']);
        } elseif ($method == 'phone') {
            return Payment::RenderPhonePayment($user->id, $this->item['code'], $for1);
        }
    }
