...
class GangMediaFile extends BaseObject
{
    public static $idField = 'id';
    public static $dataTable = 'gangfiles';
     ...
     * @return unknown_type
     */
    public static function GetGangFiles($gangId)
    {
        return parent::GetAll(self::GetDataFields(), self::$dataTable, '`gang_id`=\'' . $gangId . '\'');
    }
    ...
    {
        $query = 'SELECT count(*) FROM `' . self::$dataTable . '` WHERE `gang_id` = ' . $userId;
    ...
    public static function deleteFiles($user_id, array $items)
    {
        ...
        $items = parent::GetAll(['id', 'path'], self::$dataTable, '`gang_id`=\'' . $user_id . '\' AND `id` IN (' . $in . ')');
        unset($in);
     ...
     */
    public static function storeUploadedFile($gang_id, $file)
    {
        $data = [];
    ...
    {
        return dirname(dirname(dirname(__FILE__))) . '/' . MEDIA_FILES_PATH . '/gang/';
    }

    public static function deleteFile($gang_id, $path)
    {
        if (strpos($path, 'http://') === 0 || strpos($path, 'https://') === 0) {
            return false;
        }
        $file = self::getFolder() . $gang_id . '/' . $path;
        if (file_exists($file)) {
            return unlink($file);
        }
