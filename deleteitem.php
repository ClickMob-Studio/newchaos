...
if ($_POST['submit'] != "") {
	...
	if ($itemexist != 0) {
		...
		if ($quantity > 0) {
			perform_query("UPDATE `inventory` SET `quantity` = ? WHERE `userid` = ? AND `itemid` = ?", [$quantity, $_GET['person'], $_GET['id']]);
		} else {
			perform_query("DELETE FROM `inventory` WHERE `userid`= ? AND `itemid`= ?", [$_GET['person'], $_GET['id']]);
		}
	}
...
<tr>
	<td class="contenthead">Delete Item</td>
</tr>
<tr>
	<td class="contentcontent">
		<form method='post'>
			<table border='0' cellpadding='0' cellspacing='0'>
				<tr>
					<td colspan='2' height='27'>Are you sure you want to delete the item
						<?php echo $worked['itemname']; ?> from #<?php echo $_GET['person'] ?>?
					</td>
