...
<script>
    ...
    function applySettings() {
        ...
        // Send settings to PHP to save them
        saveSettingsToServer(username, startColor, endColor, isBold, isItalic, glow);
    }

    // Function to send settings to the server
    function saveSettingsToServer(username, startColor, endColor, isBold, isItalic, glow) {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "saveGradient.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

        const params = `username=${encodeURIComponent(username)}&startColor=${encodeURIComponent(startColor)}&endColor=${encodeURIComponent(endColor)}&bold=${isBold}&italic=${isItalic}&glow=${glow}`;
        
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                console.log(xhr.responseText); // You can show a message to the user here
            }
        };
        xhr.send(params);
    }
