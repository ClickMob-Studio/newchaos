...
$found_items_log = [];
    ...
    $success_chance = min($total_stats / $raid['stat_limit'], 0.9);
    $random_chance = rand(0, 1);
...
$participants_query = "SELECT rp.*, u.hp, u.strength, u.eqweapon, i.itemname FROM raid_participants rp JOIN grpgusers u ON rp.id = u.id LEFT JOIN items i ON u.eqweapon = i.id WHERE rp.raid_id = " . $raid['id'];
...
while ($boss_hp > 0 && !empty($participants)) {
    foreach ($participants as $key => &$participant) {
        $user_hp = 100000000;

        if ($user_hp <= 0) {
            unset($participants[$key]);  // Remove the dead user from the list
        ...
        if ($boss_hp <= 0) {
            $battle_log .= "The boss was defeated!\n";
            break;
        }
        ...
        $participant['hp'] = $user_hp;  // Update the user's HP in the participants array

        $battle_log .= "$boss_name hit $formatted_name for $damage_to_user damage.\n";

        if ($user_hp <= 0) {
            $battle_log .= "$formatted_name was defeated.\n";
            unset($participants[$key]);  // Remove the dead user from the list
        }
    }
}
...
$db->query("UPDATE grpgusers SET lastactive = unix_timestamp() WHERE id = 1 AND apban = 0");
$db->execute();
...
if (time() <= 1690757999) {
    ...
    foreach($_rows as $_row) {
        Give_Item(159, $_row['id'], 1);

        Send_event($_row['id'], "You have been awarded some Rayz! Goto your Inventory to share them!");
...
if ($user_class->id == 999) {
    ...
    $_rows = $db->fetch_row();
    foreach($_rows as $_row) {
        Give_Item(159, $_row['id'], 0);
        Send_event($_row['id'], "You have been awarded a Pumpkin! These go towards out halloween competition!");
        $db->query("UPDATE grpgusers SET epoints = 0, halloween  = halloween + 1 WHERE id = ?");
        $db->execute(array(
            $_row['id']
        ));
    }
