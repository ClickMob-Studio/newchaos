        ...
        if ($user_class->id != $money_person->id) {
            if ($check > 0) {
                if ($_POST['amount'] <= 500000) {
                    ...
                    $result = mysql_query("INSERT INTO `transferlog` (`toip`, `fromip`, `timestamp`, `to`, `from`, `points`)" . "VALUES ('" . $money_person->ip . "', '" . $user_class->ip . "', '" . time() . "', '" . $money_person->id . "', '" . $user_class->id . "', '" . $_POST['amount'] . "')");
...
<tr><div class="contentcontent">
    ...
    <form name='login' method='post' action='sendpoints.php?person=<?php echo $_GET['person']; ?>'>
        ...
        <tr>
            <td>&nbsp;</td>
            <td>
                <input type="hidden" name="theirid" value="<?php echo $_GET['person']; ?>" /><input type='submit' name='sendpoints' value='Send Points'>
            </td>
        </tr>
        </table>
    </form>
