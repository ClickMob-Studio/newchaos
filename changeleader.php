...
$result = mysql_query("UPDATE `gangs` SET `leader` = '".$new_leader->id."' WHERE `id` = '".$gang_class->id."' ");

if($_POST['leader'] != $gang_class->leader) {
$result = mysql_query("UPDATE `grpgusers` SET `grank` = '0', `gangleader` = '0' WHERE `id` = '".$gang_class->leader."'");
$result = mysql_query("UPDATE `grpgusers` SET `grank` = '1', `gangleader` = '1' WHERE `id` = '".$new_leader->id."'");
}
}
...
<tr>
<td width='15%'></td><td width='35%'><input type='submit' name='submit' value='Change Leader' /></td>
</tr>
</form>
</table>
</td></tr>
