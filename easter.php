...
<div class="floaty">
    ...
    <div style="text-align:left;padding:10px;">
        <p class="text-14"><?php echo item_popup('Easter Eggs', 157, '#7eff11') ?> will appear in your inventory and you
        ...
        <p class="text-14">So the more active you are the more <?php echo item_popup('Easter Eggs', 157, '#7eff11') ?>
            you will earn - make sure to share those Eggs!!</p>
        <!-- <h2 class="text-center mb-0">Who will you share your <?php echo item_popup('Easter Eggs', 157, '#7eff11') ?> with?</h2> -->
    </div>
    ...
    <div class="package" style="width:50%;margin:10px auto;border:1px white solid;padding:10px;">
        ...
        <p class="text-14 m-0"><?php echo item_popup('Easter Egg', 157, '#7eff11') ?> <strong>[x3]</strong> <br/>
            Cost: 100 Credits<br/>
    ...
    <?php
    $db->query("SELECT count(*) as total, `to` FROM `eventslog` WHERE text = 'You have been awarded a Easter Egg! Goto your Inventory to share the Chocolate!' GROUP BY `to` ORDER BY total DESC");
    $db->execute();
    ...
    echo '<div class="stats" style="width:90%;margin:0 auto;padding-bottom:30px">
        <h2>Statistics</h2>
        <h2 class="mb-0" style="color:#7eff11">Most Easter Eggs Earned</h2>
    ...
    foreach ($results as $result) {
        ...
        //echo '</td><td style="color:#' . $colors[$i] . ';font-weight:700;">' . item_popup('Easter Egg', 157, '#7eff11') . ' [x' . $rewards[$i] . ']</td>';
        $i++;
    }
