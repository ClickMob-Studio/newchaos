...
$conditions = [
    [$user_class->fbitime > 0, 'You can\'t perform a mug whilst in FBI Jail.'],
    [$attack_person->fbitime > 0, 'This user is currently in FBI Jail.'],
    [$attack_person->city != $user_class->city, 'You must be in the same city as the person you are mugging.'],
    [$user_class->nerve < 10 && !refill('n'), 'You need to have at least 10 nerve if you want to mug someone.'],
    [$attack_person->level < 0, 'This player is under newbie protection.'],
    [$user_class->jail > 0, 'You can\'t mug someone if you\'re in prison.'],
    [$user_class->hospital > 0, 'You can\'t mug someone if you\'re in the hospital.'],
    [empty($_GET['mug']), 'You didn\'t choose someone to mug.'],
    [$_GET['mug'] == $user_class->id, 'You can\'t mug yourself.'],
    [empty($attack_person->username), 'That person doesn\'t exist.'],
    [$attack_person->hospital > 0, 'You can\'t mug someone that\'s in hospital.'],
    [$attack_person->jail > 0, 'You can\'t mug someone that\'s in prison.'],
    [$attack_person->gang == $user_class->gang && $user_class->gang > 0, 'You can\'t mug someone that\'s in your gang.'],
    [$attack_person->id == $user_class->relplayer, 'You can\'t mug your partner.'],
    [$attack_person->mprotection > time(), 'Your target is under mug protection and cannot be mugged.'],
    [$attack_person->admin > 0, 'You can\'t mug an admin.'],
    [$user_class->mprotection > time(), 'You have an active mug protection and cannot mug during this time.']
];
...
$db->execute([$newnerve, time(), $user_class->id]);
    ...
    if ($success) {
        ...
        if ($mugamount < 1) {
            ...
            if ($user_class->gang != 0 && $gang_class->tax > 0) {
                ...
                $db->execute([$newvault, $user_class->gang]);
            }
            ...
            $db->execute([$newmuggeramount, $mugamount, $mugamount, $user_class->id]);

            $db->query("UPDATE grpgusers SET muggedmoney = muggedmoney - ?, money = ? WHERE id = ?");
            $db->execute([$mugamount, $newmuggedamount, $attack_person->id]);
            ...
            $db->execute([$user_class->id, $attack_person->id, $mugamount, $online]);
