...
function points($points, $votetokens){
	...
	if($user_class->votetokens < $votetokens)
		...
		mysql_query("UPDATE grpgusers SET points = $user_class->points, votetokens = $user_class->votetokens WHERE id = $user_class->id");
...
function money($money, $votetokens){
	...
	if($user_class->votetokens < $votetokens)
		...
		$user_class->votetokens -= $votetokens;
		mysql_query("UPDATE grpgusers SET money = $user_class->money, votetokens = $user_class->votetokens WHERE id = $user_class->id");
...
function rydays($days, $votetokens){
	...
	if($user_class->votetokens < $votetokens)
		...
		$user_class->votetokens -= $votetokens;
		mysql_query("UPDATE grpgusers SET rmdays = $user_class->rmdays, votetokens = $user_class->votetokens WHERE id = $user_class->id");
