...
$totalpts = $totalcrimes = 0;
$lastid = 0;
$var = 'busts';
$title = 'Breakout King';
for($i = 1; $i <= 72; $i++){
    $crimes = ceil((1.3 * $i * rand(50,100)) / 10);
    $pts = ceil(($crimes * rand(1200, 1750)) / 1000);
    $totalpts += $pts;
    $totalcrimes += $crimes;
    switch(rand(1,7)){
        case 1:
        ...
        case 5:
            $which = "money";
            $howmany = $pts * rand(2300, 2700);
            break;
        case 6:
        case 7:
            $which = "exp";
            $howmany = $pts * rand(400, 600);
            break;
    }
    $reqs = "$var|$crimes|$howmany|$which";
    $db->query("INSERT INTO newmissions VALUES ('', '$reqs', '$var', '$title $i', $lastid)");
	$db->execute();
    $lastid = $db->insert_id();
}
print $totalcrimes . ' = ' . $totalpts;
/*
$db->query("SELECT * FROM fiftyfiftylogs WHERE amnt = 0");
$db->execute();
$rows = $db->fetch_row();
foreach($rows as $row){
	$db->query("SELECT text FROM eventslog WHERE text LIKE '%bet%' AND (timesent = ? or timesent = ?)");
	$db->execute(array(
		$row['timestamp'] + 1,
		$row['timestamp']
	));
...
/*SSS
$db->query("UPDATE grpgusers SET eqweapon = 105 WHERE eqweapon >= 111 AND id IN (64,258,477,62,530,25,529,329)");
$db->execute();
$db->query("UPDATE grpgusers SET eqarmor = 106 WHERE eqarmor >= 111 AND id IN (64,258,477,62,530,25,529,329)");
$db->execute();
$db->query("UPDATE grpgusers SET eqshoes = 107 WHERE eqshoes >= 111 AND id IN (64,258,477,62,530,25,529,329)");
$db->execute();

/*$db->query("SELECT eqweapon, eqarmor, eqshoes, g.id, i1.itemname as weaponname, i1.image as weaponimage, i2.itemname as armorname, i2.image as armorimage, i3.itemname as shoename, i3.image as shoeimage from grpgusers g left join items i1 on i1.id = eqweapon left join items i2 on i2.id = eqarmor left join items i3 on i3.id = eqshoes where g.id IN(64,258,477,62,530,25,529,329)");
$db->execute();
$rows = $db->fetch_row();
